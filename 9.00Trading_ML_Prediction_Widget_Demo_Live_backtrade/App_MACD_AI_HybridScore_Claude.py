...
from strategies.MomentumStrategy_MACD_HybridScore_Claude import MomentumStrategy, BacktestMomentumStrategy, \
...
class TradingApp:
    def __init__(self, root):
        ...
        self.root = root
        self.root.title("v10.0-Multi-Layered Trading (MACD) Hybrid Score Claude")
        self.running = False
    ...
    def load_saved_momentum_settings(self):
        try:
            from strategies.MomentumStrategy_MACD_HybridScore_Claude import MOMENTUM_PARAMS, MomentumConfig
            self.momentum_params = MOMENTUM_PARAMS.copy()
            ...
            from strategies.MomentumStrategy_MACD_HybridScore_Claude import MOMENTUM_PARAMS
            self.momentum_params = MOMENTUM_PARAMS.copy()
            self.current_mode = "Default Parameters"
            self.custom_params = {}
            return False
    ...
    def sync_custom_params_with_code_defaults(self):
        ...
        self.log_message(f"   tier1_adx_hard_min_long     = {self.custom_params['momentum']['tier1_adx_hard_min_long']}", "green")
        self.log_message(f"   tier1_volume_min_long       = {self.custom_params['momentum']['tier1_volume_min_long']}", "green")
    ...
    def test_premium_tier1_config(self):
        ...
        checks = {
            ...
            "Tier 1 ADX Hard Min (LONG)": (s.tier1_adx_hard_min_long, 25.0),
            "Tier 1 ADX Hard Min (SHORT)": (s.tier1_adx_hard_min_short, 30.0),
            "Tier 1 RSI Min (LONG)": (s.tier1_rsi_min_long, 55.0),
            "Tier 1 RSI Max (LONG)": (s.tier1_rsi_max_long, 75.0),
            "Tier 1 RSI Min (SHORT)": (s.tier1_rsi_min_short, 25.0),
            "Tier 1 RSI Max (SHORT)": (s.tier1_rsi_max_short, 45.0),
            "Tier 1 Volume Min (LONG)": (s.tier1_volume_min_long, 1.5),
    ...
    def _get_builtin_defaults(self):
        """
        Minimal emergency fallback used ONLY if strategies.MomentumStrategy_MACD_HybridScore_Claude
        cannot be imported at all (e.g. the file is missing or has a syntax error). This is NOT
        a full parameter set -- it exists so the app can start and warn the user instead of
        crashing with an AttributeError. Fix the import error and restart as soon as possible;
        do not rely on these values for live trading.
        """
        self.log_message(
            "⚠️ Using minimal built-in defaults -- MOMENTUM_PARAMS could not be imported. "
            "Trading with this strategy is not recommended until the import error is fixed.",
            "red")
        return {
    ...
    def create_widgets(self):
        ...
        # Best Param Values button — runs strategies/Best_Param_Values.py standalone
        self.best_param_btn = tk.Button(
            date_frame,
            text="🏆 Best Params",
            command=self.run_best_param_values,
            bg="#6f42c1",
            fg="white",
            activebackground="#59339d",
        ...
        self.best_param_btn.pack(side=tk.LEFT, padx=(8, 0))
    ...
    def create_momentum_parameter_controls(self, parent):
        ...
        categories = {
            ...
            '⬆️ TIER 1 LONG FILTERS': [
                'tier1_adx_hard_min_long',
                'tier1_rsi_min_long',
                'tier1_rsi_max_long',
                'tier1_volume_min_long',
                'tier1_momentum_min',
                'tier1_macd_gate_long',
                'tier1_price_ema_max_pct',
                'daily_trend_filter_enabled',
            ],
            # Note: tier1_momentum_min is shared with LONG (sign-flipped internally),
    ...
    def _build_momentum_backtest_panel(self, right_frame):
        ...
        # ═══════════════════════════════════════════════════════════════════
        # ALL_MOMENTUM_BACKTEST_PARAMS - UPDATED v10.0.7
        # REMOVED: obsolete params (tier1_adx_hard_min, tier1_volume_min, stop_loss_atr_mult)
        # ADDED: direction-specific stop multipliers
        # ═══════════════════════════════════════════════════════════════════
        ALL_MOMENTUM_BACKTEST_PARAMS = {
            # ──────────────────────────────────────────────────────────────
            # QUALITY THRESHOLDS (tier-specific)
            # ──────────────────────────────────────────────────────────────
            'quality_tier1_min_long': {
            ...
            # ──────────────────────────────────────────────────────────────
            # TIER 1 LONG FILTERS (direction-specific)
            # ──────────────────────────────────────────────────────────────
            'tier1_adx_hard_min_long': {
                'description': 'Tier 1 ADX Hard Min (LONG)',
                'current': current_params.get('tier1_adx_hard_min_long', 25.0),
                'v1': '20', 'v2': '22', 'v3': '25', 'v4': '28'
            },
            'tier1_rsi_min_long': {
                'description': 'Tier 1 RSI Min (LONG)',
                'current': current_params.get('tier1_rsi_min_long', 55.0),
                'v1': '50', 'v2': '52', 'v3': '55', 'v4': '58'
            },
            'tier1_rsi_max_long': {
                'description': 'Tier 1 RSI Max (LONG)',
                'current': current_params.get('tier1_rsi_max_long', 75.0),
                'v1': '70', 'v2': '72', 'v3': '75', 'v4': '78'
            },
            'tier1_volume_min_long': {
                'description': 'Tier 1 Volume Min (LONG)',
                'current': current_params.get('tier1_volume_min_long', 1.5),
                'v1': '1.2', 'v2': '1.3', 'v3': '1.5', 'v4': '1.8'
            },
            'tier1_momentum_min': {
                'description': 'Tier 1 Momentum Min (shared LONG/SHORT, sign-flipped)',
            ...
            # ──────────────────────────────────────────────────────────────
            # TIER 1 SHORT FILTERS (direction-specific)
            # ──────────────────────────────────────────────────────────────
            'tier1_adx_hard_min_short': {
                'description': 'Tier 1 ADX Hard Min (SHORT)',
                'current': current_params.get('tier1_adx_hard_min_short', 30.0),
            ...
            'tier1_volume_min_short': {
                ...
                'current': current_params.get('tier1_volume_min_short', 1.3),
            ...
            'tier2_rsi_min_long': {
                ...
                'current': current_params.get('tier2_rsi_min_long', 50.0),
                'v1': '45', 'v2': '48', 'v3': '50', 'v4': '55'
            },
            'tier2_rsi_max_long': {
                'description': 'Tier 2 RSI Max (LONG)',
                'current': current_params.get('tier2_rsi_max_long', 70.0),
            ...
            # ──────────────────────────────────────────────────────────────
            # STOP LOSS (direction-specific) - REPLACED stop_loss_atr_mult
            # ──────────────────────────────────────────────────────────────
            'tier1_stop_multiplier_long': {
                'description': 'Tier 1 Stop Mult (LONG)',
                'current': current_params.get('tier1_stop_multiplier_long', 2.0),
                'v1': '1.5', 'v2': '2.0', 'v3': '2.5', 'v4': '3.0'
            },
            'tier1_stop_multiplier_short': {
                'description': 'Tier 1 Stop Mult (SHORT)',
                'current': current_params.get('tier1_stop_multiplier_short', 3.0),
                'v1': '2.0', 'v2': '2.5', 'v3': '3.0', 'v4': '4.0'
            },
            'tier2_stop_multiplier_long': {
                'description': 'Tier 2 Stop Mult (LONG)',
                'current': current_params.get('tier2_stop_multiplier_long', 2.5),
                'v1': '2.0', 'v2': '2.5', 'v3': '3.0', 'v4': '3.5'
            },
            'tier2_stop_multiplier_short': {
                'description': 'Tier 2 Stop Mult (SHORT)',
                'current': current_params.get('tier2_stop_multiplier_short', 3.5),
                'v1': '3.0', 'v2': '3.5', 'v3': '4.0', 'v4': '4.5'
            },
    ...
    def _get_current_param_value(self, param_key):
        ...
        fallbacks = {
            # Quality thresholds
            'quality_tier1_min_long': 72,
            'quality_tier2_min_long': 68,
            'quality_tier1_min_short': 75,
            'quality_tier2_min_short': 70,
            ...
            # Direction-specific stop multipliers
            ...
            # Trailing stops
            'trailing_activation_tier1': 0.04,
            'trailing_activation_tier2': 0.035,
            'trailing_distance_tier1': 0.035,
            'trailing_distance_tier2': 0.030,

            # Entry filters - LONG (direction-specific)
            'tier1_adx_hard_min_long': 20,
            'tier1_rsi_min_long': 44,
            'tier1_rsi_max_long': 64,
            'tier1_volume_min_long': 1.0,
            'tier1_momentum_min': 0.02,

            # Entry filters - SHORT (direction-specific)
            'tier1_adx_hard_min_short': 28,
            'tier1_rsi_min_short': 34,
            'tier1_rsi_max_short': 54,
            'tier1_volume_min_short': 1.3,

            # Tier 2
            'tier2_adx_hard_min': 18,
    ...
    def get_momentum_param_description(self, param_name):
        """Get description for Momentum parameters - CONSOLIDATED v10.0.7."""
        descriptions = {
            # ─── EMA ────────────────────────────────────────────────────────
            ...
            'tier1_confluence_min': 'Number of confluent signals required for Tier 1',
            'tier2_confluence_min': 'Number of confluent signals required for Tier 2',

            # ─── TIER 1 LONG ───────────────────────────────────────────────
            'tier1_adx_hard_min_long': 'Hard minimum ADX for Tier 1 LONG entries',
            'tier1_rsi_min_long': 'Minimum RSI for Tier 1 LONG entries',
            'tier1_rsi_max_long': 'Maximum RSI for Tier 1 LONG entries',
            'tier1_volume_min_long': 'Minimum volume ratio for Tier 1 LONG entries',
            'tier1_momentum_min': 'Minimum momentum for Tier 1 entries (shared, sign-flipped for SHORT)',
            'tier1_macd_gate_long': 'MACD gate required for Tier 1 entries',
            ...
            # ─── TIER 1 SHORT ──────────────────────────────────────────────
            'tier1_adx_hard_min_short': 'Hard minimum ADX for Tier 1 SHORT entries',
            'tier1_rsi_min_short': 'Minimum RSI for Tier 1 SHORT entries',
            'tier1_rsi_max_short': 'Maximum RSI for Tier 1 SHORT entries',
            'tier1_volume_min_short': 'Minimum volume ratio for Tier 1 SHORT entries',
            ...
            # ─── TIER 2 FILTERS ────────────────────────────────────────────
            'tier2_adx_hard_min': 'Hard minimum ADX for Tier 2 entries',
            'tier2_volume_min': 'Minimum volume ratio for Tier 2 entries',
            'tier2_momentum_min': 'Minimum momentum for Tier 2 entries',
            'tier2_rsi_min_long': 'Minimum RSI for Tier 2 LONG entries',
            'tier2_rsi_max_long': 'Maximum RSI for Tier 2 LONG entries',
            'tier2_rsi_min_short': 'Minimum RSI for Tier 2 SHORT entries',
            'tier2_rsi_max_short': 'Maximum RSI for Tier 2 SHORT entries',
            ...
            'tier1_stop_multiplier_long': 'Tier 1 ATR multiplier for LONG stops (stop below entry)',
            'tier1_stop_multiplier_short': 'Tier 1 ATR multiplier for SHORT stops (stop above entry)',
            'tier2_stop_multiplier_long': 'Tier 2 ATR multiplier for LONG stops (stop below entry)',
            'tier2_stop_multiplier_short': 'Tier 2 ATR multiplier for SHORT stops (stop above entry)',
            ...
            if quality_score >= 0:
                ...
                q_min = getattr(self.strategy, 'quality_min_long',
                                getattr(self.strategy, 'quality_minimum_score',
                                        getattr(self.strategy, 'quality_tier2_min_long', 75)))
    ...
    def display_trade_summary(self, trades_df, title="TRADE SUMMARY"):
        """Display a comprehensive trade summary with Tier and Direction breakdown."""
        if trades_df is None or trades_df.empty:
            self.log_message("⚠️ No trades to summarize", "orange")
            return

        self.log_message("\n" + "=" * 80, "bold cyan")
        self.log_message(f"📊 {title}", "bold cyan")
        self.log_message("=" * 80, "bold cyan")

        # ─── Overall Summary ──────────────────────────────────────────────
        total_trades = len(trades_df)
        wins = trades_df[trades_df['Win'] == 'Yes']
        losses = trades_df[trades_df['Win'] == 'No']
        win_rate = (len(wins) / total_trades * 100) if total_trades > 0 else 0

        total_pnl = trades_df['PnL'].sum() if 'PnL' in trades_df.columns else 0
        avg_return = trades_df['Return_%'].mean() if 'Return_%' in trades_df.columns else 0

        self.log_message(f"\n📈 OVERALL STATISTICS", "cyan")
        self.log_message(f"   Total Trades:     {total_trades:>6}", "white")
        self.log_message(f"   Wins:             {len(wins):>6}", "green")
        self.log_message(f"   Losses:           {len(losses):>6}", "red")
        self.log_message(f"   Win Rate:         {win_rate:>6.1f}%", "green" if win_rate >= 50 else "orange")
        self.log_message(f"   Total PnL:        ${total_pnl:>8,.2f}", "green" if total_pnl >= 0 else "red")
        self.log_message(f"   Avg Return:       {avg_return:>6.2f}%", "green" if avg_return >= 0 else "red")
        ...
        if 'Tier' in trades_df.columns:
            ...
            tiers = ['Tier 1', 'Tier 2', 'Tier 3', 'Below Tier 3', 'Unknown']
            tier_colors = {'Tier 1': 'green', 'Tier 2': 'blue', 'Tier 3': 'yellow',
                           'Below Tier 3': 'orange', 'Unknown': 'red'}

            for tier in tiers:
                tier_trades = trades_df[trades_df['Tier'] == tier]
                if len(tier_trades) == 0:
                    continue

                tier_wins = tier_trades[tier_trades['Win'] == 'Yes']
                tier_losses = tier_trades[tier_trades['Win'] == 'No']
                tier_win_rate = (len(tier_wins) / len(tier_trades) * 100) if len(tier_trades) > 0 else 0
                tier_pnl = tier_trades['PnL'].sum() if 'PnL' in tier_trades.columns else 0

                color = tier_colors.get(tier, 'white')
                self.log_message(f"   {tier}:", color)
                self.log_message(
                    f"      Trades: {len(tier_trades):>4}  |  Wins: {len(tier_wins):>4}  |  Losses: {len(tier_losses):>4}",
                    "white")
                self.log_message(f"      Win Rate: {tier_win_rate:>6.1f}%  |  PnL: ${tier_pnl:>8,.2f}", color)
                self.log_message("", "white")

        # ─── Direction Breakdown ──────────────────────────────────────────
        if 'Direction' in trades_df.columns:
            self.log_message(f"\n📊 DIRECTION BREAKDOWN", "cyan")
            self.log_message("   " + "-" * 60, "white")
            ...
            for direction in ['LONG', 'SHORT']:
                ...
                dir_wins = dir_trades[dir_trades['Win'] == 'Yes']
                dir_losses = dir_trades[dir_trades['Win'] == 'No']
                dir_win_rate = (len(dir_wins) / len(dir_trades) * 100) if len(dir_trades) > 0 else 0
                dir_pnl = dir_trades['PnL'].sum() if 'PnL' in dir_trades.columns else 0

                color = "green" if direction == 'LONG' else "red"
                emoji = "🟢" if direction == 'LONG' else "🔴"
                self.log_message(f"   {emoji} {direction}:", color)
                self.log_message(
                    f"      Trades: {len(dir_trades):>4}  |  Wins: {len(dir_wins):>4}  |  Losses: {len(dir_losses):>4}",
                    "white")
                self.log_message(f"      Win Rate: {dir_win_rate:>6.1f}%  |  PnL: ${dir_pnl:>8,.2f}", color)
                self.log_message("", "white")

        # ─── Tier × Direction Matrix ──────────────────────────────────────
        if 'Tier' in trades_df.columns and 'Direction' in trades_df.columns:
            self.log_message(f"\n📊 TIER × DIRECTION MATRIX", "cyan")
            self.log_message("   " + "-" * 75, "white")

            # Header with proper alignment
            header = f"   {'Tier':<10} {'Dir':<6} {'Trades':>8} {'Wins':>8} {'Losses':>8} {'Win%':>8} {'PnL':>12}"
            self.log_message(header, "white")
            self.log_message("   " + "-" * 75, "white")

            for tier in ['Tier 1', 'Tier 2', 'Tier 3']:
                for direction in ['LONG', 'SHORT']:
                    subset = trades_df[(trades_df['Tier'] == tier) & (trades_df['Direction'] == direction)]
                    if len(subset) == 0:
                        continue

                    wins_sub = subset[subset['Win'] == 'Yes']
                    losses_sub = subset[subset['Win'] == 'No']
                    win_rate_sub = (len(wins_sub) / len(subset) * 100) if len(subset) > 0 else 0
                    pnl_sub = subset['PnL'].sum() if 'PnL' in subset.columns else 0

                    # Determine color
                    if win_rate_sub >= 60:
                        color = "green"
                    elif win_rate_sub >= 45:
                        color = "yellow"
                    else:
                        color = "red"

                    row = f"   {tier:<10} {direction:<6} {len(subset):>8} {len(wins_sub):>8} {len(losses_sub):>8} {win_rate_sub:>7.1f}% ${pnl_sub:>11,.2f}"
                    self.log_message(row, color)
        ...
        if 'PnL' in trades_df.columns:
            ...
            self.log_message(f"\n🏆 BEST & WORST TRADES", "cyan")
            self.log_message("   " + "-" * 60, "white")

            best_trade = trades_df.loc[best_idx]
            worst_trade = trades_df.loc[worst_idx]

            best_tier = best_trade.get('Tier', 'N/A')
            best_dir = best_trade.get('Direction', '')
            worst_tier = worst_trade.get('Tier', 'N/A')
            worst_dir = worst_trade.get('Direction', '')

            self.log_message(f"   🏆 Best Trade:  ${best_trade['PnL']:>8,.2f}  ({best_tier} {best_dir})", "green")
            self.log_message(f"   💀 Worst Trade: ${worst_trade['PnL']:>8,.2f}  ({worst_tier} {worst_dir})", "red")
    ...
    def combined_constraint(self, params):
        ...
        # LONG side
        if ('quality_tier1_min_long' in self.optimization_params_active and
                'quality_tier2_min_long' in self.optimization_params_active):
            t1 = get_param(params, 'quality_tier1_min_long', 72)
            t2 = get_param(params, 'quality_tier2_min_long', 62)
            if not (t1 >= t2):
                return False

        # SHORT side (same logic)
        if ('quality_tier1_min_short' in self.optimization_params_active and
                'quality_tier2_min_short' in self.optimization_params_active):
            st1 = get_param(params, 'quality_tier1_min_short', 75)
            st2 = get_param(params, 'quality_tier2_min_short', 65)
            if not (st1 >= st2):
                return False
    ...
    def run_best_param_values(self):
        ...
        if not os.path.exists(script_path):
            self.log_message(f"❌ Best_Param_Values.py not found at: {script_path}", "red")
            messagebox.showerror(
                "File Not Found",
                f"Could not find:\n{script_path}\n\n"
                "Make sure Best_Param_Values.py exists in the 'strategies' directory."
            )
            return
    ...
    def get_optimized_params_for_export(self, optimized_params_dict):
        """Extract and format optimized parameters for saving."""
        if not optimized_params_dict:
            return {}

        # Filter to only include parameters that changed from defaults
        default_params = self.get_default_momentum_params()
        changed_params = {}
    ...
    def run_backtest(self):
        ...
        try:
            ...
            if strategy_type != "Scalping":
                strategy_module = 'strategies.MomentumStrategy_MACD_HybridScore_Claude'
            ...
            if strategy_type == "Kalman":
                ...
                # Consolidated Tier parameters (v10.0.5)
                only_tier1 = current_params.get('only_tier1_entries', False)
                tier1_long = current_params.get('quality_tier1_min_long', 'NOT FOUND')
                tier2_long = current_params.get('quality_tier2_min_long', 'NOT FOUND')
                tier1_short = current_params.get('quality_tier1_min_short', 'NOT FOUND')
                tier2_short = current_params.get('quality_tier2_min_short', 'NOT FOUND')

                self.log_message(f"   🔧 only_tier1_entries = {only_tier1}",
                                 "yellow" if only_tier1 else "green")
                self.log_message(f"   📊 Tier 1 (LONG) = {tier1_long}", "white")
                self.log_message(f"   📊 Tier 2 (LONG) = {tier2_long}", "white")
                self.log_message(f"   📊 Tier 1 (SHORT) = {tier1_short}", "white")
                self.log_message(f"   📊 Tier 2 (SHORT) = {tier2_short}", "white")
            ...
            if is_optimization:
                ...
                if strategy_type == "Kalman":
                    ...
                    try:
                        ...
                        # After optimization search completes
                        self.log_message("✅ Optimization search complete!", "green")

                        # ─── NEW: COMPARE OPTIMIZED VS BASELINE ──────────────────────────────────
                        if optimized_stats is not None and hasattr(optimized_stats, '_strategy'):
                            optimized_params_dict = {
                                k: v for k, v in optimized_stats._strategy._params.items()
                                if not k.startswith('_')
                            ...
                            if optimized_params_dict:
                                ...
                                init_equity = initial_stats.get('Equity Final [$]', 0)
                                init_sharpe = initial_stats.get('Sharpe Ratio', 0)
                                init_return = initial_stats.get('Return [%]', 0)

                                # Get optimized metrics
                                opt_equity = float(optimized_stats.get('Equity Final [$]', 0))
                                opt_sharpe = float(optimized_stats.get('Sharpe Ratio', 0))
                                opt_return = float(optimized_stats.get('Return [%]', 0))

                                # ─── COMPARE AND DECIDE ─────────────────────────────────────────
                                if opt_equity < init_equity and opt_sharpe < init_sharpe:
                                    self.log_message("=" * 70, "red")
                                    self.log_message("⚠️ OPTIMIZED RESULT IS WORSE THAN BASELINE", "red")
                                    self.log_message("=" * 70, "red")
                                    self.log_message(f"   Baseline Equity:  ${init_equity:,.2f}", "white")
                                    self.log_message(f"   Optimized Equity: ${opt_equity:,.2f}", "red")
                                    self.log_message(f"   Difference:       ${opt_equity - init_equity:,.2f}", "red")
                                    self.log_message("", "white")
                                    self.log_message("   → KEEPING BASELINE PARAMETERS", "bold cyan")
                                    self.log_message("=" * 70, "red")

                                    # Keep baseline
                                    optimized_stats = initial_stats
                                    optimized_params_dict = None
                                else:
                                    self.log_message("=" * 70, "green")
                                    self.log_message("✅ OPTIMIZED RESULT IMPROVED BASELINE", "green")
                                    self.log_message("=" * 70, "green")
                                    self.log_message(f"   Baseline Equity:  ${init_equity:,.2f}", "white")
                                    self.log_message(f"   Optimized Equity: ${opt_equity:,.2f}", "green")
                                    self.log_message(f"   Improvement:      ${opt_equity - init_equity:,.2f}", "green")
                                    self.log_message("", "white")
                                    self.log_message("   → USING OPTIMIZED PARAMETERS", "bold green")
                                    self.log_message("=" * 70, "green")

                                    # Re-run with optimized params to ensure consistency
            ...
            # In run_backtest(), after the Excel export section and before the completion message, add:

            # ─── Display Trade Summary ──────────────────────────────────────────
            self.log_message("\n" + "=" * 80, "bold purple")
            self.log_message("📊 TRADE SUMMARY", "bold purple")
            self.log_message("=" * 80, "bold purple")

            # Get trades data from the stats object
            trades_df = None

            if optimized_stats is not None and hasattr(optimized_stats, '_strategy'):
                strategy_instance = optimized_stats._strategy
            elif initial_stats is not None and hasattr(initial_stats, '_strategy'):
                strategy_instance = initial_stats._strategy
            else:
                strategy_instance = None

            if strategy_instance:
                # Try to get trade summary from strategy
                if hasattr(strategy_instance, 'get_trade_summary_dataframe'):
                    trades_df = strategy_instance.get_trade_summary_dataframe()
                elif hasattr(strategy_instance, 'trade_records') and strategy_instance.trade_records:
                    # Convert trade_records to DataFrame
                    records = strategy_instance.trade_records
                    trades_data = []
                    for rec in records:
                        if rec.exit_time is not None:  # Only closed trades
                            tier_name = f"Tier {rec.entry_tier}" if rec.entry_tier else "Unknown"
                            win = 'Yes' if rec.profit > 0 else 'No'
                            direction = rec.entry_direction.upper() if rec.entry_direction else 'UNKNOWN'
                            trades_data.append({
                                'Trade_#': rec.trade_id,
                                'Tier': tier_name,
                                'Tier_Number': rec.entry_tier or 0,
                                'Direction': direction,
                                'Entry_Price': rec.entry_price,
                                'Exit_Price': rec.exit_price,
                                'PnL': rec.profit,
                                'Return_%': rec.profit_pct,
                                'Win': win,
                                'Exit_Reason': rec.exit_reason or '',
                                'Duration': rec.hold_duration or 0,
                                'Confluence_Score': rec.confluence_score or 0,
                            })
                    if trades_data:
                        trades_df = pd.DataFrame(trades_data)
                elif hasattr(strategy_instance, 'trade_history') and strategy_instance.trade_history:
                    # Fallback: use trade_history
                    trades_data = []
                    for trade in strategy_instance.trade_history:
                        tier_num = trade.get('tier', 0)
                        tier_name = f"Tier {tier_num}" if tier_num > 0 else "Unknown"
            ...
            if trades_df is not None and not trades_df.empty:
                self.display_trade_summary(trades_df, "BACKTEST TRADE SUMMARY")
            else:
                self.log_message("⚠️ No trade data available for summary", "orange")
                    ...
                    if isinstance(val, str):
                        if val.lower() == 'true':
                            ...
                            try:
                                ...
                                pass
                    # Ensure max_daily_trades is always int
                ...
                for sheet_name in sheet_names:
                    ...
                    if sheet_name == 'Trades':
                        ...
                        if hasattr(self, 'strategy') and self.strategy is not None:
                            quality_tier1_min = getattr(self.strategy, 'quality_tier1_min_long', 72)  # v9.4.2: was 75
                            quality_tier2_min = getattr(self.strategy, 'quality_tier2_min_long', 62)  # v9.4.2: was 88
    ...
    def save_backtest_results_to_excel(self, df, stats=None, suffix=""):
        ...
        try:
            ...
            try:
                ...
                # Initialize columns (NO LONGER hardcoded to 0 - will be populated per trade)
                market_df['Entry_Signal'] = ''
                market_df['Exit_Signal'] = ''
                market_df['Exit_Reason'] = ''
                market_df['Confluence_Score'] = 0.0  # Will be overwritten at entry rows
                market_df['Risk_Allocation_%'] = 0.0  # Will be overwritten at entry rows
                    ...
                    for entry_bar, group_trades in sorted(trade_groups.items()):
                        ...
                        for leg_pos, trade in enumerate(group_trades):
                            ...
                            if entry_time is not None:
                                try:
                                    ...
                                    if entry_dt in market_df.index:
                                        ...
                                        # 🔥 NEW: Populate Confluence_Score and Risk_Allocation_% at entry row
                                        market_df.at[entry_dt, 'Confluence_Score'] = _rec_get(
                                            matching_record, 'confluence_score', default=0.0
                                        ) or 0.0
                                        market_df.at[entry_dt, 'Risk_Allocation_%'] = _rec_get(
                                            matching_record, 'risk_allocation_pct', default=0.0
                                        ) or 0.0
                                except Exception as e:
                                    pass

                                # Mark exit in market data (unchanged)
                            ...
                            trades_data.append({
                                ...
                                # 🔥 FIXED: Read from matching_record instead of hardcoding 0
                                'Confluence_Score': round(
                                    _rec_get(matching_record, 'confluence_score', default=0.0) or 0.0, 2
                                ),
                                # 🔥 FIXED: Read from matching_record instead of hardcoding 0
                                'Risk_Allocation_%': round(
                                    _rec_get(matching_record, 'risk_allocation_pct', default=0.0) or 0.0, 4
                                ),
    ...
    # NOTE: risk_allocation_pct is computed and populated by the strategy itself
    # (MomentumStrategy.calculate_position_size() -> self._entry_risk_allocation_pct
    # -> TradeRecord.risk_allocation_pct). The app reads it directly from the trade
    # record via _rec_get(matching_record, 'risk_allocation_pct', ...). The
    # _calculate_risk_allocation() helper that used to live here was dead code —
    # never called — and used a different, confluence-count-based scheme that
    # would have conflicted with the strategy's tier-based risk allocation. Removed.
    ...
    def analyze_quality_score_results(self, stats_opt):
        ...
        self.log_message(f"   Tier 1 ADX Min: {params.get('tier1_adx_hard_min_long', 25)}", "white")
        self.log_message(f"   Tier 1 RSI Range: {params.get('tier1_rsi_min_long', 35)}-{params.get('tier1_rsi_max_long', 75)}",
                         "white")
        self.log_message(f"   Tier 1 Volume Min: {params.get('tier1_volume_min_long', 1.0)}x", "white")
        self.log_message(f"\n   Tier 2 ADX Min: {params.get('tier2_adx_hard_min', 15)}", "white")
        ...
        return {
            ...
            'weights': weights,
            'tier1_adx_hard_min_long': params.get('tier1_adx_hard_min_long', 25),
            'tier1_volume_min_long': params.get('tier1_volume_min_long', 1.0),
