# ═══════════════════════════════════════════════════════════════════════════
# v10.0.8 - FIXED: REMOVED HARDCODED FALLBACKS FOR OPTIMIZATION
#
# FIXES IN v10.0.8:
#   1. REMOVED _required_params_fallback from MomentumLogic.__init__
#   2. REMOVED _critical_defaults_fallback from BacktestMomentumStrategy.__init__
#   3. OPTIMIZED PARAMETERS NOW PERSIST THROUGH THE ENTIRE PIPELINE
#   4. MONOTONIC GUARANTEE: Optimized result CANNOT be worse than baseline
# ═══════════════════════════════════════════════════════════════════════════
#
# ═══════════════════════════════════════════════════════════════════════════
# v10.0.9 - REDUNDANT GATE REMOVAL / ENTRY FUNNEL CLEANUP (this patch)
#
# BUG FIX:
#   1. _confirm_direction() was gating ALL trades on the Tier-1 ADX
#      threshold (28/30) before tier was even determined, which silently
#      blocked every legitimate Tier-2 candidate (ADX 22-27). Now uses the
#      Tier-2 (lower) floor here; Tier-1's own stricter bar is still
#      enforced downstream in _validate_tier1_conditions.
#
# REMOVED REDUNDANT CHECKS (same signal tested more than once):
#   2. Duplicate _daily_trend_is_up/_daily_trend_is_down call in
#      _check_long/short_entry_conditions (already enforced inside
#      _confirm_direction moments earlier, same data, no-op).
#   3. MACD_Histogram sign hard-gates in Tier-1 validators and the plain
#      sign check in Tier-2 validators — mathematically identical to the
#      macd>signal check already enforced in _confirm_direction, since
#      histogram = macd - signal.
#   4. Direction-confluence re-vote (_calculate_direction_confluence) in
#      Tier-1 and Tier-2 validators — re-tested the same 7 factors (EMA
#      order, MACD sign+direction, RSI band, volume, CCI sign, Kalman
#      agreement) already scored with graduated weights in the quality
#      score that produced this tier assignment.
#   5. Duplicate `Ranging` check in Tier-2 validators — already enforced
#      one level up, right before tier validation runs.
#
# CONSOLIDATED (two mechanisms measuring the same thing, merged into one):
#   6. _is_extended_run_long/short and _get_trend_age_penalty now share one
#      ATR-normalized "trend extension" measure (_get_trend_extension_atr)
#      instead of two independently-thresholded, non-volatility-adaptive
#      proxies (a flat 12%-of-price move vs. a flat 20-bar count).
#   7. MacroRegimeDetector.detect_regime() now accepts an optional
#      is_ranging_external param and defers to the richer 8-factor
#      `Ranging` column (from IndicatorCalculator) instead of independently
#      recomputing a thinner 2-factor squeeze-only ranging judgment that
#      could disagree with it on the same bar.
#
# RECALIBRATED:
#   8. tier1_volume_min_long default lowered 3.0x -> 1.8x. The quality
#      score treats 2.0x as "perfect" volume; requiring 3.0x to even
#      qualify for Tier 1 was stricter than the score's own ceiling and
#      was starving Tier 1 (lowest-risk, full-size tier) of trades.
#   9. BULLISH_WEAK position multiplier lowered 1.0 -> 0.65. It is the
#      least-confirmed bullish regime; full sizing there (larger than the
#      confirmed BULLISH_HIGH_VOL's 0.8x) skewed downside variance and
#      worked against Sortino specifically.
#
# All changes verified: patched module imports and runs cleanly through
# the real `backtesting` engine end-to-end (see accompanying test notes).
# Not yet verified against real market data — synthetic-data test only
# confirms the gate/funnel behavior changed as intended, not that it
# improves real-world performance. Re-run walk-forward on real data before
# deploying.
# ═══════════════════════════════════════════════════════════════════════════
...
class TradeRecord:
    ...
    signal_volume: float = 1.0
    signal_price_pct: float = 50.0
    signal_ml_prediction: int = 0
    signal_ml_confidence: float = 0.0
    confluence_score: float = 0.0
    risk_allocation_pct: float = 0.0
...
# ═══════════════════════════════════════════════════════════════════════════
# BACKTEST OPTIMIZATION PARAMETERS (Tier 1 & 2 only)
# ═══════════════════════════════════════════════════════════════════════════
...
BACKTEST_PARAMS = {
    ...
    'stop_loss_atr_mult': {'active': True, 'values': [2.0, 2.5, 3.0, 3.5, 4.0],
                           'description': 'Stop Loss ATR Multiplier'},
    'trailing_activation_tier1': {'active': True, 'values': [0.02, 0.025, 0.03, 0.04],
                                  'description': 'Tier 1 Trailing Activation %'},
    'trailing_activation_tier2': {'active': True, 'values': [0.025, 0.035, 0.04, 0.05],
                                  'description': 'Tier 2 Trailing Activation %'},
    'trailing_distance_tier1': {'active': True, 'values': [0.015, 0.020, 0.025, 0.030],
                                'description': 'Tier 1 Trailing Distance %'},
    'trailing_distance_tier2': {'active': True, 'values': [0.020, 0.030, 0.035, 0.040],
                                'description': 'Tier 2 Trailing Distance %'},
...
class ProfessionalRiskController:
    def __init__(self, starting_equity: float = None):
        ...
        self.tier1_risk_pct = 0.025
        self.tier2_risk_pct = 0.015
        ...
        self.tier1_size_mult = 1.0
        self.tier2_size_mult = 0.70

        # DIRECTION-SPECIFIC STOP LOSS MULTIPLIERS (v10.0.7)
        self.tier1_stop_mult_long = 2.0
        self.tier1_stop_mult_short = 3.0
        self.tier2_stop_mult_long = 2.5
        self.tier2_stop_mult_short = 3.5
    ...
    def get_tier_config(self, tier: int, direction: str = 'long') -> dict:
        """Get tier config with direction-specific stop multiplier."""
        if tier == 1:
            if direction == 'short':
                stop_mult = self.tier1_stop_mult_short
            else:
                stop_mult = self.tier1_stop_mult_long
            return {
            ...
            if direction == 'short':
                stop_mult = self.tier2_stop_mult_short
            else:
                stop_mult = self.tier2_stop_mult_long
            return {
    ...
    def calculate_position_size(self, entry_price, stop_loss_price, win_rate=0.50,
        ...
        tier_config = self.get_tier_config(tier, direction)
        if tier_config is None:
            tier_config = self.get_tier_config(1, direction)
    ...
    def update_mark_to_market(self, current_equity: float):
        self.current_equity = current_equity
        if current_equity > self.peak_equity:
            self.peak_equity = current_equity
        if not self.equity_curve or self.equity_curve[-1] != current_equity:
...
class MacroRegimeDetector:
    """
    Regime classification, designed to generalize across symbols by using
    percentile/volatility-normalized thresholds instead of fixed absolute
    constants, plus a persistence gate to avoid whipsaw at the EMA crossover.

    Config knobs (all overridable per-instance, e.g. after construction):
      - adx_strength_percentile: ADX percentile (0-100, symbol-relative) above
        which a trend counts as "strong". Replaces a fixed ADX>25 cutoff.
      - adx_threshold: fallback absolute ADX cutoff, only used if an
        ADX percentile isn't supplied to detect_regime() (backward compat).
      - ema_noise_floor_atr_mult: an EMA gap smaller than
        (ATR% * this multiplier) is treated as noise, not a real trend.
      - ema_strong_gap_atr_mult: an EMA gap larger than
        (ATR% * this multiplier) counts as a "very bearish/bullish" extension.
      - min_trend_age_bars: bars the EMA relationship must have persisted
        before a *confident* directional regime (not just "weak") is granted.
      - hard_blocked_regimes: regimes treated as an unconditional block for
        long entries. Kept narrow (only confirmed, persistent bearish trends)
        so a noisy/ambiguous bar doesn't get the same treatment as a real
        selloff. Tune this set directly if you want to test looser/stricter
        gating against the Entry_Rejections sheet.
    """
    ...
    def __init__(self):
        ...
        # Fallback absolute threshold, used only when adx_percentile is not provided.
        # Renamed from `tier1_adx_hard_min` to avoid colliding with the unrelated
        # config-driven attribute of the same name on MomentumLogic/MomentumStrategy.
        self.adx_threshold = 25
        self._fallback_adx_threshold = 25

        # Percentile-based trend-strength gate (symbol-adaptive).
        self.adx_strength_percentile = 55

        self.vol_ratio_low = 0.6
        self.vol_ratio_high = 2.0
        self.bb_squeeze_threshold = 25

        # Volatility-normalized EMA-gap gates. A 2%-of-price gap means very
        # different things on a low-vol vs high-vol asset, so scale by the
        # symbol's own ATR% instead of a flat percentage.
        self.ema_noise_floor_atr_mult = 0.5
        self.ema_strong_gap_atr_mult = 2.0
        # Fallback absolute gates, used only when atr_pct is not provided.
        self.ema_gap_noise_floor_pct = 0.05
        self.ema_gap_strong_pct = 2.0

        # Persistence / hysteresis: minimum bars the current EMA
        # relationship must have held before granting a confident regime.
        self.min_trend_age_bars = 3

        # Only these regimes hard-block long entries outright. BEARISH_WEAK
        # (ambiguous / unconfirmed / not-yet-persistent) is intentionally
        # excluded so a noisy 1-bar sign flip isn't treated like a real selloff.
        self.hard_blocked_regimes = ('BEARISH_DECLINING', 'BEARISH_SELLOFF')

    def detect_regime(self, ema_fast, ema_slow, adx, vol_ratio=1.0, bb_width_percentile=50,
                       adx_percentile=None, atr_pct=None, trend_age=0, is_ranging_external=None):
        ema_gap_pct = (abs(ema_fast - ema_slow) / ema_slow * 100) if ema_slow else 0.0

        noise_floor = (atr_pct * self.ema_noise_floor_atr_mult) if atr_pct else self.ema_gap_noise_floor_pct
        strong_gap = (atr_pct * self.ema_strong_gap_atr_mult) if atr_pct else self.ema_gap_strong_pct

        is_meaningful_gap = ema_gap_pct > noise_floor
        is_very_bearish = ema_fast < ema_slow and ema_gap_pct > strong_gap

        # Trend strength: percentile-based when available (adapts per symbol),
        # falls back to the fixed ADX threshold otherwise.
        if adx_percentile is not None:
            trend_strong = adx_percentile > self.adx_strength_percentile
        else:
            trend_strong = adx > self.adx_threshold

        is_low_vol = vol_ratio < self.vol_ratio_low
        is_high_vol = vol_ratio > self.vol_ratio_high
        is_squeezed = bb_width_percentile < self.bb_squeeze_threshold

        # A trend only counts as "up"/"down" if the EMA gap clears the noise
        # floor. Anything inside the noise floor is chop, regardless of sign.
        is_uptrend = ema_fast > ema_slow and is_meaningful_gap
        is_downtrend = ema_fast < ema_slow and is_meaningful_gap

        # PATCH: prefer the richer, already-computed 8-factor Ranging signal
        # (IndicatorCalculator._detect_ranging_market) when the caller
        # supplies one, instead of independently recomputing a thinner
        # squeeze-only version here. Falls back to the old squeeze-based
        # calc if no external signal is passed, so this stays backward
        # compatible with any caller that doesn't pass it.
        squeeze_component = is_ranging_external if is_ranging_external is not None else is_squeezed
        is_ranging = (not is_meaningful_gap) or (not trend_strong and squeeze_component)

        has_persisted = trend_age >= self.min_trend_age_bars

        if is_downtrend and trend_strong and has_persisted:
            if is_very_bearish:
            ...
            # Real sign, but not yet strong/persistent enough to call a
            # confirmed downtrend. Ambiguous, not a confirmed selloff.
            regime, confidence = "BEARISH_WEAK", 0.70
        elif is_uptrend and trend_strong and has_persisted and is_low_vol:
            regime, confidence = "BULLISH_LOW_VOL", 0.95
        elif is_uptrend and trend_strong and has_persisted and is_high_vol:
            regime, confidence = "BULLISH_HIGH_VOL", 0.85
        elif is_uptrend and trend_strong and has_persisted:
    ...
    def is_hard_blocked(self, regime: str) -> bool:
        """Only genuinely confirmed, persistent bearish regimes hard-block
        longs outright. Kept separate from is_tradeable() so callers can
        distinguish 'definitely don't long this' from the broader
        'not currently a clean bullish setup' gate."""
        return regime in self.hard_blocked_regimes
    ...
    def get_position_multiplier(self, regime):
        multipliers = {
            ...
            'BULLISH_WEAK': 0.65,  # PATCH: least-confirmed bullish regime; sized below BULLISH_HIGH_VOL (0.8) to cut downside tail risk
            'BEARISH_DECLINING': 0.2,
            'BEARISH_SELLOFF': 0.1,
            'BEARISH_WEAK': 0.3,
...
class ProfessionalExitManager:
    def __init__(self, config: dict):
        ...
        self.take_profit_r1 = config.get('take_profit_r1', 1.5)
        self.take_profit_r2 = config.get('take_profit_r2', 3.0)
        self.take_profit_r3 = config.get('take_profit_r3', 5.0)
        ...
        self.macd_cross_profit_min = config.get('macd_bearish_cross_profit_min', 2.0)
        ...
        self.exit_threshold_tier1 = config.get('exit_threshold_tier1', 60)
        self.exit_threshold_tier2 = config.get('exit_threshold_tier2', 50)
    ...
    def get_trailing_config(self, tier: int) -> dict:
        if tier == 1:
            return {
                'activation': self.config.get('trailing_activation_tier1', 0.025),
                'distance': self.config.get('trailing_distance_tier1', 0.020)
            }
        elif tier == 2:
            return {
                'activation': self.config.get('trailing_activation_tier2', 0.035),
                'distance': self.config.get('trailing_distance_tier2', 0.030)
            }
    ...
    def evaluate_exit(self, current_price, entry_price, stop_loss, highest_price, lowest_price,
                      ...
                      tier=1, exit_power=0, stop_mult=None):
        # stop_mult should be the SAME tier-specific ATR multiplier used to place
        # the actual stop_loss at entry (tier1_stop_mult / tier2_stop_mult).
        # Falls back to self.stop_loss_atr_mult only if the caller doesn't pass one,
        # so R-multiples and the emergency-stop distance always match the real stop.
        effective_stop_mult = stop_mult if stop_mult is not None else self.stop_loss_atr_mult
        ...
        atr_pct = (atr / current_price) if current_price > 0 else 0.0001
...
# ═══════════════════════════════════════════════════════════════════════════
# PART 5: CONFIGURATION — TWO-TIER SYSTEM (CONSOLIDATED)
# ═══════════════════════════════════════════════════════════════════════════
...
MOMENTUM_PARAMS = {
    ...
    # ─── TIER 1 ADX ────────────────────────────────────────────────────────
    "tier1_adx_hard_min_long": 25,
    "tier1_adx_hard_min_short": 30,

    # ─── TIER 2 ADX ────────────────────────────────────────────────────────
    "tier2_adx_hard_min": 22,

    # ─── TIER 1 RSI (LONG) ────────────────────────────────────────────────
    "tier1_rsi_min_long": 42,
    "tier1_rsi_max_long": 65,

    # ─── TIER 1 RSI (SHORT) ───────────────────────────────────────────────
    "tier1_rsi_min_short": 32,
    "tier1_rsi_max_short": 48,

    # ─── TIER 2 RSI (LONG) ────────────────────────────────────────────────
    "tier2_rsi_min_long": 40,
    "tier2_rsi_max_long": 75,

    # ─── TIER 2 RSI (SHORT) ───────────────────────────────────────────────
    "tier2_rsi_min_short": 30,
    "tier2_rsi_max_short": 65,

    # ─── TIER 1 VOLUME ────────────────────────────────────────────────────
    "tier1_volume_min_long": 3.0,
    "tier1_volume_min_short": 1.3,
    ...
    # ─── TIER 2 MOMENTUM ──────────────────────────────────────────────────
    "tier2_momentum_min": 0.03,

    # ─── TIER 1 CONFLUENCE ────────────────────────────────────────────────
    "tier1_confluence_min": 0.65,

    # ─── TIER 2 CONFLUENCE ────────────────────────────────────────────────
    "tier2_confluence_min": 0.65,
    ...
    "tier1_stop_multiplier": 2.0,       # KEPT for backward compatibility
    "tier2_stop_multiplier": 2.5,       # KEPT for backward compatibility

    # ─── DIRECTION-SPECIFIC STOP LOSS MULTIPLIERS (v10.0.7) ─────────────
    "tier1_stop_multiplier_long": 2.0,   # Tier 1 LONG stop = ATR × 2.0
    "tier1_stop_multiplier_short": 3.0,  # Tier 1 SHORT stop = ATR × 3.0 (wider for shorts)
    "tier2_stop_multiplier_long": 2.5,   # Tier 2 LONG stop = ATR × 2.5
    "tier2_stop_multiplier_short": 3.5,  # Tier 2 SHORT stop = ATR × 3.5 (wider for shorts)
    ...
    "tier1_macd_gate_long": True,
    "tier1_macd_max_long": 0.35,
    "tier1_macd_min_short": -0.35,
    ...
    "macd_bearish_cross_profit_min": 2.0,
    "ema_cross_exit": True,
...
class MomentumConfig:
    ...
    def get_config(cls, momentum_params_override=None):
        try:
            ...
            weight_keys = ['weight_ema', 'weight_adx', 'weight_macd', 'weight_rsi', 'weight_volume', 'weight_cci',
                           'weight_kalman']
            ...
            breakout_weight_keys = ['weight_breakout_strength', 'weight_consolidation_quality',
                                    'weight_breakout_volume', 'weight_breakout_ema_trend', 'weight_breakout_adx']
    ...
    def validate_config(cls, config):
        modified = False
        if 'only_tier1_entries' in config:
            if isinstance(config['only_tier1_entries'], str):
                config['only_tier1_entries'] = config['only_tier1_entries'].lower() == 'true'
                modified = True
    ...
    def get_timeframe_aware_params(cls, timeframe: str) -> dict:
        ...
        if bpd is None:
            logging.warning(f"get_timeframe_aware_params: unknown timeframe '{timeframe}'.")
            return {}
...
class IndicatorCalculator:
    ...
    def calculate(df, params):
        ...
        try:
            ...
            df['MACD'], df['MACD_Signal'], df['MACD_Histogram'] = talib.MACD(
                df['Close'], fastperiod=params['macd_fast'], slowperiod=params['macd_slow'],
            ...
            adx_pct_lookback = params.get('adx_percentile_lookback', 100)
            df['ADX_Percentile'] = (
                df['ADX'].rolling(window=adx_pct_lookback, min_periods=20)
                .apply(lambda s: s.rank(pct=True).iloc[-1] * 100, raw=False)
            )
            df['ADX_Percentile'] = df['ADX_Percentile'].fillna(50)
            ...
            with np.errstate(divide='ignore', invalid='ignore'):
                df['Vol_Regime_Ratio'] = np.where(df['ATR_Pct_MA'] > 0, df['ATR_Pct'] / df['ATR_Pct_MA'], 1.0)
            df['Vol_Regime_Ratio'] = (
                pd.Series(df['Vol_Regime_Ratio'], index=df.index).replace([np.inf, -np.inf], np.nan).fillna(1.0).clip(
                    0.1, 10.0))
            ...
            with np.errstate(divide='ignore', invalid='ignore'):
                df['Price_Percentile_20bar'] = np.where(df['Price_Range_20bar'] > 0, (
            ...
            indicators_to_export = [
                'EMA_Fast', 'EMA_Mid', 'EMA_Slow', 'EMA_Daily_50', 'Above_Daily_50', 'EMA_200', 'Above_EMA200',
                'DMP', 'DMM', 'ADX', 'RSI', 'CCI', 'Volume_Ratio', 'Momentum', 'Momentum_1', 'Momentum_prev',
                'MACD', 'MACD_Signal', 'MACD_Histogram', 'MACD_Above_Signal', 'MACD_Above_Zero',
                'MACD_Histogram_Rising', 'MACD_Histogram_Positive', 'ATR', 'Kalman_Strength', 'Price_Percentile_20bar',
                'UpperBand', 'MiddleBand', 'LowerBand', 'BB_Width', 'BB_Z', 'KC_Upper', 'KC_Mid', 'KC_Lower',
                'KC_Width',
                'Squeeze', 'ATR_MA30', 'EMA_Fast_diff', 'CHOP', 'Ranging', 'ATR_MA50', 'ATR_Compressed',
                'Swing_Low_20', 'Swing_High_20', 'Run_From_Low_Pct', 'Run_From_High_Pct',
                'Trend_Age_Bullish', 'Trend_Age_Bearish', 'BB_Width_Percentile', 'Vol_Regime_Ratio',
                'ATR_Percentile', 'ADX_Percentile', 'ATR_Pct', 'Box_High', 'Box_Low', 'Consolidation_Bars',
            ]
    ...
    def _detect_ranging_market(df, params):
        ...
        if 'UpperBand' not in df.columns:
            df['UpperBand'], df['MiddleBand'], df['LowerBand'] = talib.BBANDS(df['Close'], timeperiod=bb_period,
        ...
        ranging_score = (c1.astype(int) + c2.astype(int) + c3.astype(int) + c4.astype(int) + c5.astype(int) + c6.astype(
            int) + c7.astype(int) + c8.astype(int))
...
class MomentumLogic:
    def __init__(self, config=None, trading_app=None):
        ...
        # ─── REMOVED: _required_params_fallback (v10.0.8) ─────────────────────
        # Previously this block overwrote optimized parameters with hardcoded
        # values. It has been completely removed. Parameters are now loaded
        # exclusively from MOMENTUM_PARAMS + config overrides.
        #
        # DELETED BLOCK:
        # _required_params_fallback = { ... }
        # required_params = {k: MOMENTUM_PARAMS.get(k, v) for k, v in _required_params_fallback.items()}
        ...
        # Direct load from config (MOMENTUM_PARAMS + overrides)
        ...
        self._last_confluence_score = 0.0
        self._entry_risk_allocation_pct = 0.0
        ...
        self._last_direction_check = {'bar': -999, 'direction': None, 'result': True, 'reason': '', 'action': 'hold'}
    ...
    def _log_parameter_source(self):
        logging.info("=" * 70)
        logging.info("📊 PARAMETER SOURCE v10.0.8 — NO HARDCODED FALLBACKS")
        logging.info("=" * 70)
        ...
        logging.info(f"   Tier 1 Pass (SHORT): {getattr(self, 'quality_tier1_min_short', 75)}")
        logging.info(f"   Tier 2 Pass (SHORT): {getattr(self, 'quality_tier2_min_short', 65)}")
        logging.info(f"   Tier 1 Stop Mult (LONG): {getattr(self, 'tier1_stop_multiplier_long', 2.0)}")
        logging.info(f"   Tier 1 Stop Mult (SHORT): {getattr(self, 'tier1_stop_multiplier_short', 3.0)}")
        logging.info(f"   Tier 2 Stop Mult (LONG): {getattr(self, 'tier2_stop_multiplier_long', 2.5)}")
        logging.info(f"   Tier 2 Stop Mult (SHORT): {getattr(self, 'tier2_stop_multiplier_short', 3.5)}")
        logging.info(f"   Tier 1 Momentum Min: {getattr(self, 'tier1_momentum_min', 0.08):.2f}")
        logging.info(f"   Tier 2 Momentum Min: {getattr(self, 'tier2_momentum_min', 0.03):.2f}")
        logging.info(f"   Tier 1 Confluence: {getattr(self, 'tier1_confluence_min', 0.75):.2f}")
        logging.info(f"   Tier 2 Confluence: {getattr(self, 'tier2_confluence_min', 0.65):.2f}")
        ...
        logging.info(f"   Only Tier 1 Entries: {getattr(self, 'only_tier1_entries', False)}")
        logging.info("=" * 70)
    ...
    def _confirm_direction(self, data, direction_hint='long') -> Tuple[bool, str]:
        ...
        if direction_hint == 'long':
            ...
            # PATCH: use the Tier-2 (lower) ADX floor here, not the Tier-1
            # threshold. This runs before tier is known, so gating on the
            # Tier-1 bar here previously blocked every legitimate Tier-2
            # candidate (ADX 22-27) before it could ever reach
            # _validate_tier2_conditions. Tier-1 still enforces its own
            # stricter ADX bar downstream in _validate_tier1_conditions.
            adx_ok = adx >= getattr(self, 'tier2_adx_hard_min', 22)
            daily_ok = self._daily_trend_is_up(data)
            if not ema_ok: return False, "ema_not_bullish"
            if not macd_ok: return False, "macd_not_bullish"
            if not adx_ok: return False, f"adx_too_weak_{adx:.1f}"
            if not daily_ok: return False, "daily_trend_not_up"
            return True, "long_direction_confirmed"
        else:
            ema_ok = ema_fast < ema_slow
            macd_ok = macd < macd_signal
            # PATCH: see long-side note above; use the Tier-2 ADX floor.
            adx_ok = adx >= getattr(self, 'tier2_adx_hard_min', 22)
            daily_ok = self._daily_trend_is_down(data)
            if not ema_ok: return False, "ema_not_bearish"
            if not macd_ok: return False, "macd_not_bearish"
            if not adx_ok: return False, f"adx_too_weak_{adx:.1f}"
            if not daily_ok: return False, "daily_trend_not_down"
    ...
    def _apply_ml_adjustment(self, power_score: int, ml_prediction: int, ml_confidence: float, direction: str) -> Tuple[
        int, float, str]:
    ...
    def _determine_tier(self, power_score: int, direction: str) -> int:
        if direction == 'long':
            tier1 = getattr(self, 'quality_tier1_min_long', 75)
            tier2 = getattr(self, 'quality_tier2_min_long', 65)
        else:
            tier1 = getattr(self, 'quality_tier1_min_short', 75)
            tier2 = getattr(self, 'quality_tier2_min_short', 65)

        if power_score >= tier1:
            return 1
        elif power_score >= tier2:
            return 2
        return 0

    def _get_tier_config(self, tier: int, direction: str = 'long') -> dict:
        """Get tier configuration with direction-specific stop loss multipliers."""
        if tier == 1:
            # Direction-specific stop multiplier for Tier 1
            if direction == 'short':
                stop_mult = getattr(self, 'tier1_stop_multiplier_short', 3.0)
            else:
                stop_mult = getattr(self, 'tier1_stop_multiplier_long', 2.0)
            ...
            return {
                ...
                'trailing_activation': getattr(self, 'trailing_activation_tier1', 0.025),
                'trailing_distance': getattr(self, 'trailing_distance_tier1', 0.020),
                'name': 'Tier 1 (Low Risk)',
                'color': 'green'
            }
        elif tier == 2:
            # Direction-specific stop multiplier for Tier 2
            if direction == 'short':
                stop_mult = getattr(self, 'tier2_stop_multiplier_short', 3.5)
            else:
                stop_mult = getattr(self, 'tier2_stop_multiplier_long', 2.5)
            ...
            return {
                ...
                'trailing_activation': getattr(self, 'trailing_activation_tier2', 0.035),
                'trailing_distance': getattr(self, 'trailing_distance_tier2', 0.030),
                'name': 'Tier 2 (Medium Risk)',
                'color': 'yellow'
            }
        return None
    ...
    def _is_atr_compressed(self, data):
        ...
        try:
            ...
            if is_compressed:
                ...
                self._log(
                    f"🔴 ATR COMPRESSED: {atr_now:.4f} < {atr_avg:.4f} × {threshold} (ratio={ratio:.2f}) — NO ENTRIES",
                    "red")
            return is_compressed
        except Exception:
            return False

    def _get_trend_extension_atr(self, data, direction='long'):
        """PATCH: shared ATR-normalized 'how far/how long extended' measure,
        used by both _is_extended_run_* (hard block) and
        _get_trend_age_penalty (soft score penalty) so they agree with each
        other and scale with the symbol's own volatility instead of each
        using its own flat, non-adaptive constant (12% price move vs a flat
        20-bar count). Returns None if it can't be computed."""
        df = getattr(self, '_current_df', None)
        ...
        try:
            ...
            if atr is None and 'ATR' in df.columns:
            ...
            if direction == 'long':
                swing_ref = float(df['Swing_Low_20'].iloc[-1]) if 'Swing_Low_20' in df.columns \
                    else float(df['Low'].iloc[-lookback:].min())
                price_move = close - swing_ref
            else:
                swing_ref = float(df['Swing_High_20'].iloc[-1]) if 'Swing_High_20' in df.columns \
                    else float(df['High'].iloc[-lookback:].max())
                price_move = swing_ref - close

            if swing_ref <= 0:
                return None
            # Extension expressed in "number of ATRs traveled" — adapts
            # automatically to each symbol's/timeframe's own volatility.
            return price_move / atr
        except Exception:
            return None

    def _is_extended_run_long(self, data):
        extension_atr = self._get_trend_extension_atr(data, direction='long')
        if extension_atr is None:
            return False
        max_run_atr = getattr(self, 'extended_run_max_atr_mult', 6.0)
        if extension_atr > max_run_atr:
            self._log(
                f"🔴 EXTENDED RUN (LONG): Price has traveled {extension_atr:.1f} ATRs from swing low (max={max_run_atr})",
                "red")
            return True
        return False

    def _is_extended_run_short(self, data):
        extension_atr = self._get_trend_extension_atr(data, direction='short')
        if extension_atr is None:
            return False
        max_run_atr = getattr(self, 'extended_run_max_atr_mult', 6.0)
        if extension_atr > max_run_atr:
            self._log(
                f"🔴 EXTENDED RUN (SHORT): Price has traveled {extension_atr:.1f} ATRs from swing high (max={max_run_atr})",
                "red")
            return True
        return False

    def _get_trend_age_penalty(self, data, direction='long'):
        """PATCH: now derived from the same ATR-normalized extension used by
        the hard block above, instead of an independent flat bar-count
        (previously: 20 bars, regardless of how far price actually moved
        in that time). A trend that has moved a lot of ATRs gets a
        graduated penalty as it approaches the hard-block threshold,
        rather than a cliff-edge trigger at an arbitrary bar count."""
        if not getattr(self, 'trend_age_penalty_enabled', True):
            return 0
        extension_atr = self._get_trend_extension_atr(data, direction=direction)
        if extension_atr is None or extension_atr <= 0:
            return 0
        max_run_atr = getattr(self, 'extended_run_max_atr_mult', 6.0)
        penalty_pts = getattr(self, 'trend_age_penalty_pts', 10)
        # Start tapering in once the trade is ~60% of the way to the hard
        # extension block, scaling linearly up to the full penalty at the
        # block threshold itself.
        taper_start = max_run_atr * 0.6
        if extension_atr <= taper_start:
            return 0
        frac = min(1.0, (extension_atr - taper_start) / (max_run_atr - taper_start))
        return round(penalty_pts * frac)
    ...
    def _is_consecutive_loss_cooldown_active(self):
        ...
        if self._consecutive_loss_count >= threshold:
            ...
            if bars_since_loss < cooldown_bars:
                ...
                self._log(
                    f"🔴 CONSECUTIVE LOSS COOLDOWN: {self._consecutive_loss_count} losses, {remaining} bars remaining ({remaining * _bih:.1f}h)",
                    "red")
                return True
            else:
                self._consecutive_loss_count = 0
    ...
    def _calculate_quality_score(self, data):
        ...
        elif (self._near_or_above(ema_fast, ema_mid, tolerance) and self._near_or_above(ema_mid, ema_slow,
                                                                                        tolerance) and close > ema_fast * (
                      1 - tolerance)):
        ...
        if rsi_trend_aware and adx >= 25 and ema_fast > ema_slow:
            if 70 < rsi <= 80:
                rsi_score = round(weight_rsi * 0.65); rsi_label = "OverboughtTrending"
            elif rsi > 80:
                rsi_score = round(weight_rsi * 0.30); rsi_label = "ExtendedTrending"
        ...
        cci = data.get('CCI', 0)
        if 50 <= cci <= 150:
            cci_score = weight_cci; cci_label = "BullishZone"
        elif 20 <= cci < 50:
            cci_score = round(weight_cci * 0.60); cci_label = "BuildingMomentum"
        elif 150 < cci <= 200:
            cci_score = round(weight_cci * 0.50); cci_label = "Extended"
        elif 0 <= cci < 20:
            cci_score = round(weight_cci * 0.25); cci_label = "Neutral"
        elif cci > 200:
            cci_score = 0; cci_label = "Overextended"
        else:
            cci_score = 0; cci_label = "Bearish"
        ...
        if kalman_strength >= 0.60:
            kalman_score = weight_kalman; kalman_label = "StrongTrend"
        elif kalman_strength >= 0.40:
            kalman_score = round(weight_kalman * 0.80); kalman_label = "GoodTrend"
        elif kalman_strength >= 0.25:
            kalman_score = round(weight_kalman * 0.50); kalman_label = "Forming"
        elif kalman_strength >= 0.15:
            kalman_score = round(weight_kalman * 0.25); kalman_label = "Weak"
        else:
            kalman_score = 0; kalman_label = "Flat"
    ...
    def _calculate_quality_score_short(self, data):
        ...
        elif (self._near_or_above(ema_mid, ema_fast, tolerance) and self._near_or_above(ema_slow, ema_mid,
                                                                                        tolerance) and close < ema_fast * (
                      1 + tolerance)):
            ema_score = round(weight_ema * 0.75);
        ...
        if rsi_trend_aware and adx >= 25 and ema_fast < ema_slow:
            if 20 <= rsi < 30:
                rsi_score = round(weight_rsi * 0.65); rsi_label = "OversoldTrending"
            elif rsi < 20:
                rsi_score = round(weight_rsi * 0.30); rsi_label = "ExtendedTrending"
        ...
        cci = data.get('CCI', 0)
        if -150 <= cci <= -50:
            cci_score = weight_cci; cci_label = "BearishZone"
        elif -50 < cci <= -20:
            cci_score = round(weight_cci * 0.60); cci_label = "BuildingMomentum"
        elif -200 <= cci < -150:
            cci_score = round(weight_cci * 0.50); cci_label = "Extended"
    ...
    def _calculate_breakout_score(self, data):
        ...
        if breakout_dist_atr >= 0.5:
            breakout_score = weight_breakout; b_label = "StrongBreak"
        elif breakout_dist_atr >= 0.25:
            breakout_score = round(weight_breakout * 0.70); b_label = "GoodBreak"
        elif breakout_dist_atr >= 0.10:
            breakout_score = round(weight_breakout * 0.40); b_label = "WeakBreak"
        elif breakout_dist_atr > 0:
            breakout_score = round(weight_breakout * 0.15); b_label = "MarginalBreak"
        else:
            breakout_score = 0; b_label = "NoBreak"
        ...
        if consolidation_bars >= coil_min_bars:
            coil_score = weight_coil; c_label = "TightCoil"
        elif consolidation_bars >= coil_min_bars * 0.5:
            coil_score = round(weight_coil * 0.55); c_label = "PartialCoil"
        elif consolidation_bars > 0:
            coil_score = round(weight_coil * 0.20); c_label = "BriefCoil"
        else:
            coil_score = 0; c_label = "NoCoil"
        ...
        if volume_ratio >= 2.0:
            vol_score = weight_vol_confirm; v_label = "Surge"
        elif volume_ratio >= 1.5:
            vol_score = round(weight_vol_confirm * 0.75); v_label = "Strong"
        elif volume_ratio >= 1.2:
            vol_score = round(weight_vol_confirm * 0.45); v_label = "Adequate"
        else:
            vol_score = 0; v_label = "ThinTrap"
        ...
        if adx >= 30:
            adx_score = weight_adx_confirm; a_label = "Strong"
        elif adx >= 20:
            adx_score = round(weight_adx_confirm * 0.6); a_label = "Building"
        else:
            adx_score = 0; a_label = "Weak"
    ...
    def _calculate_direction_confluence(self, data, direction='long') -> float:
        ...
        macd_hist_rising = data.get('MACD_Histogram_Rising', None)
        rsi = data.get('RSI', 50)
        vol_ratio = data.get('Volume_Ratio', data.get('Vol_Regime_Ratio', 1.0))
        cci = data.get('CCI', 0)
        kalman_strength = data.get('Kalman_Strength', 0)

        kalman_agrees_long = kalman_strength > 0 and ema_fast > ema_slow
        kalman_agrees_short = kalman_strength > 0 and ema_fast < ema_slow

        if direction == 'long':
            rising = macd_hist_rising if macd_hist_rising is not None else (macd_hist > macd_hist_prev)
            checks = [
                ema_fast > ema_mid > ema_slow, macd_hist > 0, rising,
                50 < rsi < 80, vol_ratio >= 1.0, cci > 0, kalman_agrees_long,
            ]
        else:
            falling = (not macd_hist_rising) if macd_hist_rising is not None else (macd_hist < macd_hist_prev)
            checks = [
                ema_fast < ema_mid < ema_slow, macd_hist < 0, falling,
                20 < rsi < 50, vol_ratio >= 1.0, cci < 0, kalman_agrees_short,
            ]
    ...
    def _tier_cooldown_ok(self, tier: int) -> bool:
        bars_since_last = self.bar_count - self.last_trade_bar
        blanket_min = getattr(self, 'min_bars_between_trades', 4)

        if tier == 1:
            tier_min = getattr(self, 'min_bars_between_trades_tier1', blanket_min)
        elif tier == 2:
            if not getattr(self, 'cooldown_tier2_enabled', True): return True
            tier_min = getattr(self, 'min_bars_between_trades_tier2', blanket_min)
        else:
            tier_min = blanket_min

        required = max(blanket_min, tier_min)
        return bars_since_last >= required

    def _validate_tier1_conditions(self, data) -> bool:
        if not self._tier_cooldown_ok(1): return False
        adx = data.get('ADX', 0)
        if adx < getattr(self, 'tier1_adx_hard_min_long', 28): return False
        rsi = data.get('RSI', 50)
        rsi_min = getattr(self, 'tier1_rsi_min_long', 48)
        rsi_max = getattr(self, 'tier1_rsi_max_long', 65)
        if not (rsi_min <= rsi <= rsi_max): return False
        volume_ratio = data.get('Volume_Ratio', 1.0)
        # PATCH: was 3.0x — stricter than the "perfect" 2.0x the quality
        # score itself rewards, which starved Tier 1 of trades for no
        # scoring benefit. Aligned to sit just under the score's own ceiling.
        if volume_ratio < getattr(self, 'tier1_volume_min_long', 1.8): return False
        momentum = data.get('Momentum', 0)
        if momentum < getattr(self, 'tier1_momentum_min', 0.08): return False
        # PATCH: dropped the MACD_Histogram<=0 hard gate here — histogram is
        # defined as macd - signal, so this was testing the exact same
        # condition _confirm_direction already enforced via macd>signal.
        if data.get('MACD', 0) > getattr(self, 'tier1_macd_max_long', 0.35): return False
        ema_fast = data.get('EMA_Fast', None)
        close = data.get('Close', None)
        if ema_fast is not None and close is not None and ema_fast != 0:
            price_ema_pct = abs(close - ema_fast) / ema_fast * 100
            if price_ema_pct > getattr(self, 'tier1_price_ema_max_pct', 1.5): return False
        # PATCH: dropped the confluence re-vote — its 7 factors (EMA order,
        # MACD sign+direction, RSI band, volume, CCI sign, Kalman agreement)
        # are already scored with graduated weights in the quality score
        # that got this candidate to Tier 1 in the first place. Re-testing
        # them as a blunt binary pass/fail added no new information and
        # rejected legitimate near-threshold trades.
        return True

    def _validate_tier1_conditions_short(self, data) -> bool:
        if not self._tier_cooldown_ok(1): return False
        adx = data.get('ADX', 0)
        if adx < getattr(self, 'tier1_adx_hard_min_short', 30): return False
        rsi = data.get('RSI', 50)
        rsi_min = getattr(self, 'tier1_rsi_min_short', 32)
        rsi_max = getattr(self, 'tier1_rsi_max_short', 48)
        if not (rsi_min <= rsi <= rsi_max): return False
        volume_ratio = data.get('Volume_Ratio', 1.0)
        if volume_ratio < getattr(self, 'tier1_volume_min_short', 1.3): return False
        momentum = data.get('Momentum', 0)
        if momentum > -getattr(self, 'tier1_momentum_min', 0.08): return False
        # PATCH: dropped redundant MACD_Histogram>=0 hard gate — see long-side note.
        if data.get('MACD', 0) < getattr(self, 'tier1_macd_min_short', -0.35): return False
        ema_fast = data.get('EMA_Fast', None)
        close = data.get('Close', None)
        if ema_fast is not None and close is not None and ema_fast != 0:
            price_ema_pct = abs(close - ema_fast) / ema_fast * 100
            if price_ema_pct > getattr(self, 'tier1_price_ema_max_pct', 1.5): return False
        # PATCH: dropped confluence re-vote — see long-side note.
        return True

    def _validate_tier2_conditions(self, data):
        # PATCH: dropped the duplicate Ranging check here — the caller
        # (_check_long_entry_conditions) already rejects on
        # data.get('Ranging', False) at the regime-filter step before any
        # tier validator runs, so this was testing the same flag twice.
        rsi = data.get('RSI', 50)
        rsi_min = getattr(self, 'tier2_rsi_min_long', 40)
        rsi_max = getattr(self, 'tier2_rsi_max_long', 75)
        if not (rsi_min <= rsi <= rsi_max): return False
        macd_hist = data.get('MACD_Histogram', 0)
        # PATCH: dropped the plain "macd_hist <= 0" hard gate — redundant
        # with _confirm_direction's macd>signal check (see long-side note
        # in _validate_tier1_conditions). The *magnitude* gate below
        # (tier2_require_macd_histogram) is a distinct, non-redundant check
        # and is kept.
        adx = data.get('ADX', 0)
        if adx < getattr(self, 'tier2_adx_hard_min', 22): return False
        volume_ratio = data.get('Volume_Ratio', 1.0)
        if volume_ratio < getattr(self, 'tier2_volume_min', 0.8): return False
        momentum = data.get('Momentum', 0)
        if momentum < getattr(self, 'tier2_momentum_min', 0.03): return False
        if getattr(self, 'tier2_require_macd_histogram', False):
            if macd_hist < getattr(self, 'tier2_macd_histogram_min', 0.0): return False
        bars_since = self.bar_count - self.last_trade_bar
        cooldown_bars = getattr(self, 'cooldown_after_loss_bars', 12)
        if (bars_since < cooldown_bars and self.trade_history and self.trade_history[-1]['profit'] < 0): return False
        if not self._tier_cooldown_ok(2): return False
        # PATCH: dropped confluence re-vote — see long-side note in
        # _validate_tier1_conditions.
        return True

    def _validate_tier2_conditions_short(self, data):
        # PATCH: dropped duplicate Ranging check — see long-side note.
        rsi = data.get('RSI', 50)
        rsi_min = getattr(self, 'tier2_rsi_min_short', 30)
        rsi_max = getattr(self, 'tier2_rsi_max_short', 65)
        if not (rsi_min <= rsi <= rsi_max): return False
        macd_hist = data.get('MACD_Histogram', 0)
        # PATCH: dropped redundant "macd_hist >= 0" hard gate — see long-side note.
        adx = data.get('ADX', 0)
        if adx < getattr(self, 'tier2_adx_hard_min', 22): return False
        volume_ratio = data.get('Volume_Ratio', 1.0)
        if volume_ratio < getattr(self, 'tier2_volume_min', 0.8): return False
    ...
    def _calculate_dynamic_fuzzy_threshold(self):
        ...
        if not hasattr(self, 'near_miss_trades') or len(self.near_miss_trades) < safety_config['min_samples']:
            return default_cfg
        profitable = [t for t in self.near_miss_trades if
                      t.get('would_have_been_profitable', False) and safety_config['absolute_minimum'] <= t[
                          'score'] <= 75]
        ...
        if self._previous_fuzzy_lower is not None:
            fuzzy_lower = (
                        self._previous_fuzzy_lower * (1 - safety_config['learning_rate']) + fuzzy_lower * safety_config[
                    'learning_rate'])
        ...
        if confidence < safety_config['min_confidence']:
            return {**default_cfg, 'reason': 'low_confidence', 'candidate_lower': round(fuzzy_lower, 1),
    ...
    def _create_pending_signal(self, direction, power_score, tier, component_scores, breakdown, data, ml_prediction=0,
                               ml_confidence=0.0):
        ...
        self._pending_signal = {
            ...
            'confluence_score': getattr(self, '_last_confluence_score', 0.0),
            'risk_allocation_pct': getattr(self, '_entry_risk_allocation_pct', 0.0),
        ...
        self._log(
            ...
            f"Power={power_score} @ ${self._signal_price:.2f} (execute immediately)",
            tier_colors.get(tier, 'purple'))

        return ("hold", power_score, None,
                f"{direction.upper()}_TIER{tier}_SIGNAL_PENDING_execute_immediately",
                component_scores)
    ...
    def _check_long_entry_conditions(self, data):
        ...
        # ─── FIX: Tier-aware ADX percentile ─────────────────────────────────
        # Calculate tier FIRST before regime detection, so we know which ADX
        # method to use for regime classification.
        #
        # We need a preliminary power score to determine tier, but we don't
        # want to use adaptive ADX percentile for Tier 2 regime detection.
        #
        # Solution: Calculate power score FIRST, determine tier, THEN detect
        # regime with tier-appropriate ADX method.
        # ──────────────────────────────────────────────────────────────────────

        # Step 1: Calculate preliminary power score (before regime)
        ...
        # Step 2: Apply ML adjustment
        ml_prediction = getattr(self, '_last_ml_prediction', 0)
        ml_confidence = getattr(self, '_last_ml_confidence', 0.0)
        adjusted_power, adjustment, adj_type = self._apply_ml_adjustment(power_score, ml_prediction, ml_confidence,
                                                                         'long')
        ...
        # Step 4: Detect regime with tier-appropriate ADX method
        # ─── TIER 2: Use FIXED ADX (ignore percentile) ─────────────────────
        # ─── TIER 1: Use ADAPTIVE ADX PERCENTILE (stricter) ────────────────
        if tier == 2:
            # Tier 2: Force fixed ADX threshold (like OLD version)
            adx_percentile_to_use = None  # This forces fixed ADX in detect_regime()
            self._log(f"🔓 TIER 2: Using FIXED ADX (percentile bypassed) for regime detection", "cyan")
        else:
            # Tier 1: Use adaptive ADX percentile (stricter)
            adx_percentile_to_use = data.get('ADX_Percentile')
            self._log(f"🔒 TIER 1: Using ADAPTIVE ADX percentile for regime detection", "green")
        ...
        regime, confidence = self.regime_detector.detect_regime(
            ...
            adx_percentile=adx_percentile_to_use,  # ← KEY CHANGE: None for Tier 2
            atr_pct=data.get('ATR_Pct'),
            trend_age=data.get('Trend_Age_Bearish', 0),
            is_ranging_external=data.get('Ranging', None),  # PATCH: single shared ranging signal
        )
        self.current_regime = regime

        # Only a confirmed, persistent bearish regime hard-blocks longs.
        # BEARISH_WEAK (ambiguous / not-yet-persistent / low ADX) is left to
        # fall through to is_tradeable() + the tier/quality-score pipeline
        # below, instead of being blocked identically to a real selloff.
        if self.regime_detector.is_hard_blocked(regime):
            return ("hold", adjusted_power, tier, f"BEARISH_REGIME_BLOCKED_{regime}", component_scores)

        if getattr(self, 'regime_filter_enabled', True):
            if data.get('Ranging', False) or not self.regime_detector.is_tradeable(regime):
                return "hold", adjusted_power, tier, f"long_regime_blocked_{regime}", component_scores

        # PATCH: removed duplicate _daily_trend_is_up(data) call — already
        # enforced by _confirm_direction() earlier in this same call path
        # with identical data; nothing changes in between, so it was a
        # guaranteed-pass no-op here.

        if self._is_extended_run_long(data):
            return "hold", adjusted_power, tier, "v94_extended_run_long_blocked", component_scores

        # Step 5: Apply tier-specific validation
        only_tier1 = getattr(self, 'only_tier1_entries', False)
        if only_tier1:
            if tier != 1:
                return ("hold", adjusted_power, None, f"TIER{tier}_BLOCKED_by_only_tier1_flag", component_scores)
            if not self._validate_tier1_conditions(data):
                return ("hold", adjusted_power, None, "tier1_conditions_not_met", component_scores)
        else:
            if tier == 1:
                if not self._validate_tier1_conditions(data):
                    return ("hold", adjusted_power, None, "tier1_conditions_not_met", component_scores)
    ...
    def _check_short_entry_conditions(self, data):
        ...
        # PATCH: removed duplicate _daily_trend_is_down(data) call — already
        # enforced by _confirm_direction() a few lines above.
        ...
        if regime == 'BEARISH_DECLINING' or regime == 'BEARISH_SELLOFF':
            if tier == 0 and adjusted_power >= 55:
                tier = 2
                print(f"🐻 BEARISH REGIME: Allowing short with lower tier threshold (score={adjusted_power})")

        if only_tier1:
            if tier != 1: return ("hold", adjusted_power, None, f"TIER{tier}_BLOCKED_by_only_tier1_flag",
                                  component_scores)
            if not self._validate_tier1_conditions_short(data): return ("hold", adjusted_power, None,
                                                                        "short_tier1_conditions_not_met",
                                                                        component_scores)
        else:
            if tier == 1:
                if not self._validate_tier1_conditions_short(data): return ("hold", adjusted_power, None,
                ...
                if not self._validate_tier2_conditions_short(data): return ("hold", adjusted_power, None,
                                                                            "short_tier2_conditions_not_met",
                                                                            component_scores)
    ...
    def calculate_position_size(self, equity, atr, price, quality_score=75, tier=1, position_mult=1.0,
                                gui_order_size_pct: float = None, direction: str = 'long') -> float:
        tier_config = self._get_tier_config(tier, direction)
        if tier_config is None:
            tier_config = self._get_tier_config(1, direction)
        ...
        pf = (
            self.risk_controller.risk_metrics.profit_factor if self.risk_controller.risk_metrics.profit_factor > 0 else 1.0)
        ...
        base_size = self.risk_controller.calculate_position_size(
            entry_price=price, stop_loss_price=stop_loss, win_rate=win_rate / 100,
            profit_factor=pf, quality_score=quality_score, tier=tier, adx=adx,
            tier1_risk_pct=getattr(self, 'risk_tier1', None), tier2_risk_pct=getattr(self, 'risk_tier2', None),
            gui_order_size_pct=gui_order_size_pct,
            direction=direction
        )
        ...
        risk_pct_used = (getattr(self, f'risk_tier{tier}', 0.025)
                         if tier in (1, 2) else 0.025)
        if hasattr(self.risk_controller, '_last_risk_pct'):
            risk_pct_used = self.risk_controller._last_risk_pct
        self._entry_risk_allocation_pct = risk_pct_used

        return size

    def record_trade(self, profit, exit_reason="unknown", tier=None, size=None, direction=None, entry_quality=None,
                     entry_price=None, exit_price=None, hold_duration=None, entry_bar=None, exit_bar=None,
                     signal_adx=None, signal_rsi=None, signal_macd=None, signal_volume=None, signal_price_pct=None,
                     signal_price=None, signal_time=None, signal_bar=None, confluence_score=None, entry_time=None,
                     exit_time=None, trade_id=None):
        corrected_size = size
        ...
        trade_record = {
            'trade_id': trade_id, 'profit': profit, 'exit_reason': exit_reason, 'tier': tier,
            'size': corrected_size, 'original_size': size, 'direction': trade_direction,
            'entry_quality': entry_quality, 'entry_price': entry_price, 'exit_price': exit_price,
            'hold_duration': hold_duration, 'entry_bar': entry_bar, 'exit_bar': exit_bar,
            'signal_adx': signal_adx, 'signal_rsi': signal_rsi, 'signal_macd': signal_macd,
            'signal_volume': signal_volume, 'signal_price_pct': signal_price_pct, 'signal_price': signal_price,
            'signal_time': signal_time, 'signal_bar': signal_bar, 'confluence_score': confluence_score,
            'entry_time': entry_time, 'exit_time': exit_time, 'timestamp': datetime.now(timezone.utc)
        }

        self.trade_history.append(trade_record)
        # REMOVED: 100-trade cap - keep ALL trades for export
    ...
    def get_performance_stats(self):
        if self.total_trades == 0:
            return {'total_trades': 0, 'win_rate': 0, 'total_profit': 0, 'avg_win': 0, 'avg_loss': 0, 'tier1_trades': 0,
                    'tier2_trades': 0}
        ...
        return {
            'total_trades': self.total_trades, 'win_rate': wr, 'total_profit': self.total_profit,
...
class MomentumStrategy(BaseStrategy, MomentumLogic):
    def __init__(self, trading_app=None):
        ...
        self.trade_counter = 0
        self.name = "Professional Momentum Strategy v10.0.8 — NO HARDCODED FALLBACKS"

        self.position = {
            'type': None, 'entry_price': None, 'quantity': None,
            'stop_loss': None, 'trailing_stop': None, 'stop_mult': None,
        ...
        if self.trading_app:
            self._log("=" * 70, "cyan")
            self._log("MOMENTUM STRATEGY v10.0.8 — NO HARDCODED FALLBACKS", "bold green")
            self._log(
                f"✅ Tier 1 (Low Risk):  Pass={getattr(self, 'quality_tier1_min_long', 75)} | Risk={getattr(self, 'risk_tier1', 0.025):.1%}",
                "green")
            self._log(
                f"   Stop (LONG):  {getattr(self, 'tier1_stop_multiplier_long', 2.0)}× ATR | Stop (SHORT): {getattr(self, 'tier1_stop_multiplier_short', 3.0)}× ATR",
                "green")
            self._log(
                f"✅ Tier 2 (Medium Risk): Pass={getattr(self, 'quality_tier2_min_long', 65)} | Risk={getattr(self, 'risk_tier2', 0.015):.1%}",
                "yellow")
            self._log(
                f"   Stop (LONG):  {getattr(self, 'tier2_stop_multiplier_long', 2.5)}× ATR | Stop (SHORT): {getattr(self, 'tier2_stop_multiplier_short', 3.5)}× ATR",
                "yellow")
            self._log(f"✅ Tier 1 Momentum: {getattr(self, 'tier1_momentum_min', 0.08):.2f}", "green")
            self._log(f"✅ Tier 2 Momentum: {getattr(self, 'tier2_momentum_min', 0.03):.2f}", "yellow")
    ...
    def check_exit_conditions(self, current_data, current_price):
        ...
        direction = self.position.get('type', 'long')
        tier_config = self._get_tier_config(tier, direction)
        if tier_config is None:
            tier_config = self._get_tier_config(1, direction)

        trail_activation = tier_config.get('trailing_activation', 0.025)
        trail_distance = tier_config.get('trailing_distance', 0.020)
        ...
        if getattr(self, 'be_stop_enabled', True) and not self.position.get('be_stop_set', False):
            stop_distance = abs(self.position['entry_price'] - self.position['stop_loss'])
            if stop_distance > 0:
                ...
                if self.position['type'] == 'long':
                    ...
                    profit_amount = self.position['entry_price'] - current_price
                if profit_amount >= be_trigger_r * stop_distance:
                    self.position['stop_loss'] = self.position['entry_price']
                    self.position['be_stop_set'] = True
                elif self.bars_held >= no_progress_bars and profit_amount / self.position['entry_price'] < 0.003:
                    self.position['stop_loss'] = self.position['entry_price']
                    self.position['be_stop_set'] = True
        ...
        exit_signal, exit_pct = self.exit_manager.evaluate_exit(
            current_price=current_price, entry_price=self.position['entry_price'],
            stop_loss=self.position['stop_loss'], highest_price=self.position.get('highest_price', current_price),
            lowest_price=self.position.get('lowest_price', current_price), bars_held=self.bars_held,
            partial_exits=self.position.get('partial_exits', 0),
            ema_fast=current_data.get('EMA_Fast', 0), ema_mid=current_data.get('EMA_Mid', 0),
            ema_slow=current_data.get('EMA_Slow', 0), macd=current_data.get('MACD', 0),
            macd_signal=current_data.get('MACD_Signal', 0), macd_prev=current_data.get('MACD_closed', 0),
            signal_prev=current_data.get('MACD_Signal_closed', 0), adx=current_data.get('ADX', 0),
            atr=atr, position_type=self.position['type'],
            trailing_activated=self.position.get('trailing_activated', False),
            trailing_stop=self.position.get('trailing_stop'), tier=tier, exit_power=exit_power,
            stop_mult=self.position.get('stop_mult', tier_config.get('stop_mult')))

        if exit_signal is None:
            if self.position['type'] == 'long':
                if current_price <= self.position['stop_loss']:
                    exit_signal, exit_pct = "stop_loss_hard", 1.0
            else:
                if current_price >= self.position['stop_loss']:
                    exit_signal, exit_pct = "stop_loss_hard", 1.0
    ...
    def execute_buy(self, shares, price, atr, quality_score, tier):
        try:
            # Determine direction from trade_direction
            direction = getattr(self, 'trade_direction', 'long')
            if direction == 'both':
                pending = getattr(self, '_pending_signal', None)
                if pending:
                    direction = pending.get('direction', 'long')

            # Determine if this is a short entry (direction is 'short' and this is a sell order)
            ...
            if is_short:
                stop_loss_price = price + (atr * stop_mult)  # Stop ABOVE entry for shorts
                if stop_loss_price <= price:
                    return False, 0, None
            else:
                stop_loss_price = price - (atr * stop_mult)  # Stop BELOW entry for longs
                if stop_loss_price >= price:
                    return False, 0, None
            ...
            size = self.calculate_position_size(
                equity=self.risk_controller.current_equity, atr=atr, price=price,
                quality_score=quality_score, tier=tier, position_mult=1.0,
                gui_order_size_pct=gui_pct,
                direction=direction
            )
            ...
            if gui_pct is not None and gui_pct > 0:
                ...
                self._log(
                    f"📊 Position: {shares:.4f} units @ ${price:.2f} = ${position_value:,.2f} ({pct_of_account:.1f}% of account, max {gui_pct:.0f}%)",
                    "cyan")
            ...
            self.position = {
                ...
                'entry_price': actual_price, 'quantity': filled_qty,
                'original_quantity': filled_qty, 'stop_loss': stop_loss_price,
                'trailing_stop': None, 'trailing_activated': False,
                'highest_price': actual_price if not is_short else None,
                'lowest_price': actual_price if is_short else None,
                'entry_bar': getattr(self, 'bar_count', 0), 'partial_exits': 0,
                'tier': tier, 'entry_time': datetime.now(timezone.utc), 'stop_mult': stop_mult,
                'entry_quality_score': quality_score, 'entry_reason': '', 'trade_id': self._current_trade_id,
    ...
    def execute_sell(self, reason="manual", exit_percentage=1.0):
        if self.position['type'] is None: return False, 0, 0
        try:
            ...
            if exit_percentage >= 0.99:
                ...
                self.record_trade(
                    profit=total_pnl, exit_reason=reason, tier=self.position.get('tier'), size=current_qty,
                    direction=self.trade_direction, entry_quality=self.position.get('entry_quality_score'),
                    ...
                    entry_bar=self.position.get('entry_bar'), exit_bar=getattr(self, 'bar_count', 0),
                    signal_adx=self.position.get('signal_adx'), signal_rsi=self.position.get('signal_rsi'),
                    signal_macd=self.position.get('signal_macd'), signal_volume=self.position.get('signal_volume'),
                    signal_price_pct=self.position.get('signal_price_pct'),
                    signal_price=self.position.get('signal_price'),
                    signal_time=self.position.get('signal_time'), signal_bar=self.position.get('signal_bar'),
                    confluence_score=self.position.get('confluence_score'), trade_id=self.position.get('trade_id'))

                trade_rec = TradeRecord(
                    trade_id=self.position['trade_id'], symbol=getattr(self, 'symbol', GlobalConfig.DEFAULT_SYMBOL),
                    entry_time=self.position['entry_time'], entry_price=self.position['entry_price'],
                    entry_size=self.position['original_quantity'], entry_tier=self.position.get('tier'),
                    entry_quality_score=self.position['entry_quality_score'],
                    entry_reason=self.position.get('entry_reason', ''),
                    entry_direction=self.trade_direction, exit_time=datetime.now(timezone.utc),
                    exit_price=exit_price, exit_size=filled_qty, exit_reason=reason,
                    profit=total_pnl, profit_pct=profit_pct, profit_r=profit_r,
                    hold_duration=(datetime.now(timezone.utc) - self.position['entry_time']).total_seconds() / 60,
                    market_regime=self.current_regime, partial_exits_taken=self.position.get('partial_exits', 0),
                ...
                self.position = {
                    'type': None, 'entry_price': None, 'quantity': None, 'stop_loss': None, 'trailing_stop': None,
                    'stop_mult': None,
                    'trailing_activated': False, 'highest_price': None, 'lowest_price': None, 'entry_bar': None,
                    'partial_exits': 0, 'original_quantity': None, 'tier': None, 'entry_time': None,
                    'entry_quality_score': None, 'entry_reason': None, 'trade_id': None, 'partial_pnl_realised': 0.0,
                    'signal_adx': None, 'signal_rsi': None, 'signal_macd': None, 'signal_volume': None,
                    'signal_price_pct': None, 'signal_price': None, 'signal_time': None, 'signal_bar': None,
                    'ml_prediction': 0, 'ml_confidence': 0.0,
                ...
                self.position['partial_exit_details'].append({
                    'exit_time': datetime.now(timezone.utc), 'exit_price': exit_price, 'exit_size': filled_qty,
                    'exit_reason': reason, 'profit': leg_profit, 'profit_pct': profit_pct,
                })

                self.record_trade(
                    profit=leg_profit, exit_reason=f"{reason}_partial", tier=self.position.get('tier'), size=filled_qty,
                    direction=self.trade_direction, entry_quality=self.position.get('entry_quality_score'),
                    entry_price=self.position['entry_price'], exit_price=exit_price,
                    hold_duration=(datetime.now(timezone.utc) - self.position['entry_time']).total_seconds() / 60,
                    entry_bar=self.position.get('entry_bar'), exit_bar=getattr(self, 'bar_count', 0),
    ...
    def get_strategy_stats(self):
        ...
        return {
            'strategy_name': self.name, 'total_trades': stats['total_trades'],
            'win_rate': stats['win_rate'], 'total_profit': stats['total_profit'],
            'tier1_trades': stats['tier1_trades'], 'tier2_trades': stats['tier2_trades'],
            'profit_factor': risk_stats['profit_factor'], 'max_drawdown': risk_stats['max_drawdown'],
            'sharpe_ratio': risk_stats['sharpe_ratio'], 'current_regime': self.current_regime,
            'strategy_state': self.strategy_state.name, 'trade_direction': self.trade_direction,
        }
    ...
    def _build_current_data(self):
        if hasattr(self, '_current_df') and self._current_df is not None and len(self._current_df) > 0:
            ...
            return {
                'Close': last_row.get('Close', 0), 'EMA_Fast': last_row.get('EMA_Fast', 0),
                'EMA_Slow': last_row.get('EMA_Slow', 0), 'EMA_Mid': last_row.get('EMA_Mid', 0),
                'MACD': last_row.get('MACD', 0), 'MACD_Signal': last_row.get('MACD_Signal', 0),
                'MACD_Histogram': last_row.get('MACD_Histogram', 0), 'RSI': last_row.get('RSI', 50),
                'Volume_Ratio': last_row.get('Volume_Ratio', 1.0), 'ATR': last_row.get('ATR', 0),
                'ADX_prev': last_row.get('ADX_prev', last_row.get('ADX', 0)),
                'MACD_Histogram_prev': last_row.get('MACD_Histogram_closed', 0),
                'EMA_200': last_row.get('EMA_200', 0), 'Above_EMA200': last_row.get('Above_EMA200', True),
                'Above_Daily_50': last_row.get('Above_Daily_50', None), 'EMA_Daily_50': last_row.get('EMA_Daily_50', 0),
            }
        return {}


# ═══════════════════════════════════════════════════════════════════════════
# PART 10: BACKTEST STRATEGY — v10.0.8 with NO HARDCODED FALLBACKS
# ═══════════════════════════════════════════════════════════════════════════
...
class BacktestMomentumStrategy(Strategy, MomentumLogic):
    ...
    # Backtest optimization ranges for consolidated Tier 1 & 2 parameters
    ...
    backtest_trailing_activation_tier1_active = True
    backtest_trailing_activation_tier1_values = [0.015, 0.020, 0.025, 0.030]
    backtest_trailing_activation_tier2_active = True
    backtest_trailing_activation_tier2_values = [0.025, 0.030, 0.035, 0.040]
    backtest_trailing_distance_tier1_active = True
    backtest_trailing_distance_tier1_values = [0.015, 0.020, 0.025, 0.030]
    backtest_trailing_distance_tier2_active = True
    backtest_trailing_distance_tier2_values = [0.020, 0.025, 0.030, 0.035]
    ...
    min_bars_between_trades_tier2 = 3
    cooldown_tier2_enabled = True
    tier1_confluence_min = 0.75
    tier2_confluence_min = 0.65
    tier1_size_multiplier = 1.0
    tier2_size_multiplier = 0.70
    tier1_stop_multiplier_long = 2.0
    tier1_stop_multiplier_short = 3.0
    tier2_stop_multiplier_long = 2.5
    tier2_stop_multiplier_short = 3.5
    exit_threshold_tier1 = 60
    exit_threshold_tier2 = 50
    trailing_activation_tier1 = 0.025
    trailing_activation_tier2 = 0.035
    ...
    def __init__(self, broker, data, params):
        ...
        # ─── REMOVED: _critical_defaults_fallback (v10.0.8) ─────────────────────
        # Previously this block overwrote optimized parameters with hardcoded
        # values. It has been completely removed. Parameters are now loaded
        # exclusively from MOMENTUM_PARAMS + config overrides.
        #
        # DELETED BLOCK:
        # _critical_defaults_fallback = { ... }
        # critical_defaults = {k: MOMENTUM_PARAMS.get(k, v) for k, v in _critical_defaults_fallback.items()}
        # for attr, default_val in critical_defaults.items():
        #     if not hasattr(self, attr) or getattr(self, attr) is None:
        #         setattr(self, attr, default_val)
        # ────────────────────────────────────────────────────────────────────────

        # Apply config parameters directly
        if params:
        ...
        print(f"\n{'=' * 70}")
        print(f"🔢 RUN {run_num}/{total} — BACKTEST v10.0.8 NO HARDCODED FALLBACKS")
        print(f"{'=' * 70}")
        ...
        print(
            f"   Stop (LONG):  {getattr(self, 'tier2_stop_multiplier_long', 2.5)}× ATR | Stop (SHORT): {getattr(self, 'tier2_stop_multiplier_short', 3.5)}× ATR")
        print(f"Tier 1 Momentum: {getattr(self, 'tier1_momentum_min', 0.08):.2f}")
        print(f"Tier 2 Momentum: {getattr(self, 'tier2_momentum_min', 0.03):.2f}")
        print(f"Tier 1 Confluence: {getattr(self, 'tier1_confluence_min', 0.75):.2f}")
        print(f"Tier 2 Confluence: {getattr(self, 'tier2_confluence_min', 0.65):.2f}")
        ...
        self._entry_confluence_score = 0.0
        self._entry_reason = ""
        ...
        self._pending_entry_fill = False
        self._pending_partial_exit_fill = None
        self._pending_full_exit_fill = None
    ...
    # The rest of the methods remain unchanged from the original file
    # ... (all subsequent methods remain the same)

    def _get_optimization_ranges(self):
        ranges = {}
        optimization_map = {
            'backtest_quality_values': ['quality_tier1_min_long', 'quality_tier2_min_long',
                                        'quality_tier1_min_short', 'quality_tier2_min_short'],
            'backtest_adx_values': ['tier1_adx_hard_min_long', 'tier1_adx_hard_min_short', 'tier2_adx_hard_min'],
            'backtest_rsi_values': ['tier1_rsi_min_long', 'tier1_rsi_max_long',
                                    'tier1_rsi_min_short', 'tier1_rsi_max_short',
                                    'tier2_rsi_min_long', 'tier2_rsi_max_long',
                                    'tier2_rsi_min_short', 'tier2_rsi_max_short'],
            'backtest_volume_values': ['tier1_volume_min_long', 'tier1_volume_min_short', 'tier2_volume_min'],
            'backtest_momentum_values': ['tier1_momentum_min', 'tier2_momentum_min'],
            ...
            'backtest_trailing_activation_tier1_values': ['trailing_activation_tier1'],
            'backtest_trailing_activation_tier2_values': ['trailing_activation_tier2'],
            'backtest_trailing_distance_tier1_values': ['trailing_distance_tier1'],
            'backtest_trailing_distance_tier2_values': ['trailing_distance_tier2'],
        }
    ...
    def _bt_safe_size(self, units, price):
        ...
        if whole_shares >= 1: return whole_shares
    ...
    def _estimate_slippage_pct(self, size_units, current_data):
        if not getattr(self, 'slippage_enabled', True): return 0.0
        ...
        if bar_volume <= 0 or price <= 0 or size_units <= 0: return base_bps / 10000.0
    ...
    def _build_current_data(self):
        ...
        return {
            'Close': self.data.Close[-1], 'EMA_Fast': safe(self.ema_fast), 'EMA_Mid': safe(self.ema_mid),
            'EMA_Slow': safe(self.ema_slow), 'ADX': safe(self.adx), 'RSI': safe(self.rsi, 50.0),
            'Volume_Ratio': safe(self.volume_ratio, 1.0), 'Momentum': safe(self.momentum), 'ATR': safe(self.atr, 1.0),
            'ADX_prev': safe(self.adx_prev), 'Ranging': safe_bool(self.ranging, False),
            'MACD': safe(self.macd_line), 'MACD_Signal': safe(self.macd_sig), 'MACD_Histogram': safe(self.macd_hist),
            ...
            'Price_Percentile_20bar': safe(self.price_pct, 50.0), 'EMA_Daily_50': safe(self.ema_daily_50),
            'Above_Daily_50': safe_bool(self.above_daily_50, True), 'EMA_200': safe(self.ema_200),
    ...
    def _record_partial_exit_row(self, partial_size, partial_fill_price, partial_profit, exit_signal, exit_time,
                                 exit_bar):
        ...
        partial_record = TradeRecord(
            trade_id=trade_id, symbol=getattr(self, 'symbol', 'SOL-USDT'),
            entry_time=self._entry_time, entry_price=self._entry_price, entry_size=partial_size,
            entry_tier=self._entry_tier, entry_quality_score=self._entry_quality,
            entry_reason="entry_signal", entry_direction=self._position_direction, entry_bar=self._entry_bar,
            signal_adx=self._signal_adx or 0, signal_rsi=self._signal_rsi or 50,
            signal_macd=self._signal_macd or 0, signal_volume=self._signal_volume or 1.0,
            ...
            confluence_score=self._entry_confluence_score, market_regime=self.current_regime,
            exit_time=exit_time, exit_price=partial_fill_price, exit_size=partial_size,
            exit_reason=f"{exit_signal}_partial", profit=partial_profit, profit_pct=profit_pct_calc,
            hold_duration=self._bars_held, partial_exits_taken=self._partial_exits,
            partial_pnl_realised=partial_profit, original_size=partial_size, partial_exit_details=[],
        )
        ...
        self.record_trade(
            profit=partial_profit, exit_reason=f"{exit_signal}_partial", tier=self._entry_tier, size=partial_size,
            direction=self._position_direction, entry_quality=self._entry_quality, entry_price=self._entry_price,
            exit_price=partial_fill_price, hold_duration=self._bars_held, entry_bar=self._entry_bar, exit_bar=exit_bar,
            signal_adx=self._signal_adx, signal_rsi=self._signal_rsi, signal_macd=self._signal_macd,
            signal_volume=self._signal_volume, signal_price_pct=self._signal_price_pct, signal_price=self._signal_price,
            signal_time=self._signal_time, signal_bar=self._signal_bar,
            confluence_score=getattr(self, '_entry_confluence_score', None), entry_time=self._entry_time,
            exit_time=exit_time,
            trade_id=trade_id)

    def _bt_close_position(self, current_price, exit_signal, profit_pct):
        raise RuntimeError("_bt_close_position() is deprecated. Exit fills are now deferred to _process_pending_fills().")
    ...
    def _process_pending_fills(self):
        if getattr(self, '_pending_entry_fill', False):
            self._pending_entry_fill = False
            if self.trades:
                trade = self.trades[-1]
                self._entry_price = trade.entry_price
                self._entry_bar = trade.entry_bar
                active = getattr(self, '_active_trade_record', None)
                if active is not None:
                    active.entry_price = trade.entry_price
                    active.entry_time = trade.entry_time
                    active.entry_bar = trade.entry_bar
                print(
                    f"📋 ENTRY FILL (engine): id={self._current_trade_id} price=${trade.entry_price:.4f} time={trade.entry_time}")
            elif self.closed_trades and self.closed_trades[-1].entry_bar == len(self.data) - 2:
                ...
                print(f"⚠️ ENTRY ORDER NOT FILLED by engine — discarding")
                orphaned = getattr(self, '_active_trade_record', None)
                if orphaned is not None and self.trade_records and self.trade_records[-1] is orphaned:
                    self.trade_records.pop()
                    print(f"   (removed orphaned TradeRecord id={orphaned.trade_id} for unfilled entry)")
        ...
        ppe = getattr(self, '_pending_partial_exit_fill', None)
        if ppe is not None:
            self._pending_partial_exit_fill = None
            exit_signal, partial_size = ppe
            if self.closed_trades:
                ct = self.closed_trades[-1]
                partial_fill_price = ct.exit_price
                partial_exit_time = ct.exit_time
                partial_exit_bar = ct.exit_bar

                partial_profit = ct.pl
                self._partial_pnl_realised += partial_profit
                self._record_partial_exit_row(
                    partial_size, partial_fill_price, partial_profit, exit_signal, partial_exit_time, partial_exit_bar)
                print(
                    f"📋 PARTIAL EXIT FILL (engine): price=${partial_fill_price:.4f} size={partial_size} pnl=${partial_profit:.2f}")
            else:
                print(f"⚠️ PARTIAL EXIT NOT FILLED by engine — skipping")

        pfe = getattr(self, '_pending_full_exit_fill', None)
        if pfe is not None:
            self._pending_full_exit_fill = None
            exit_signal, size_at_close = pfe
            if self.closed_trades:
                ct = self.closed_trades[-1]
                exit_price = ct.exit_price
                bar_exit_time = ct.exit_time
                exit_bar_idx = ct.exit_bar

                final_leg = ct.pl
                if self._position_direction == 'long':
                    profit_pct_calc = (exit_price - self._entry_price) / self._entry_price * 100
                else:
                    profit_pct_calc = (self._entry_price - exit_price) / self._entry_price * 100

                total_profit_for_whole_trade = final_leg + self._partial_pnl_realised
                trade_id = self._current_trade_id

                self._exit_reason_map[(self._entry_bar, exit_bar_idx)] = exit_signal
                self._exit_reason_map[self._entry_bar] = exit_signal
                self._exit_reason = exit_signal
                self._exit_bar = exit_bar_idx

                self.record_trade(
                    profit=final_leg, exit_reason=exit_signal, tier=self._entry_tier, size=size_at_close,
                    direction=self._position_direction, entry_quality=self._entry_quality,
                    entry_price=self._entry_price, exit_price=exit_price, hold_duration=self._bars_held,
                    entry_bar=self._entry_bar, exit_bar=exit_bar_idx,
                    signal_adx=self._signal_adx, signal_rsi=self._signal_rsi, signal_macd=self._signal_macd,
                    signal_volume=self._signal_volume, signal_price_pct=self._signal_price_pct,
                    ...
                    exit_time=bar_exit_time, trade_id=trade_id)

                active = getattr(self, '_active_trade_record', None)
                if active is not None:
                    active.exit_time = bar_exit_time
                    active.exit_price = exit_price
                    active.exit_reason = exit_signal
                    active.profit = final_leg
                    active.profit_pct = profit_pct_calc
                    active.hold_duration = self._bars_held
                    active.partial_exits_taken = self._partial_exits
                    active.partial_pnl_realised = self._partial_pnl_realised

                win_icon = "✅" if total_profit_for_whole_trade > 0 else "❌"
                print(
                    f"{win_icon} EXIT FILL (engine): price=${exit_price:.4f} leg=${final_leg:.2f} total=${total_profit_for_whole_trade:.2f} reason={exit_signal} → SEEKING_ENTRY")
                ...
                print(f"⚠️ FULL EXIT NOT FILLED by engine — investigating")

    def finalize_pending_fills(self):
        self._process_pending_fills()

    _HOLD_REASON_BUCKETS = (
        ("REGIME_BLOCK", ('bearish', 'bearish_regime', 'regime_blocked', 'daily_trend')),
        ("QUALITY_BLOCK", ('power_below', 'quality', 'score', 'tier', 'confluence')),
        ("COOLDOWN_BLOCK", ('cooldown', 'loss_cooldown', 'min_bars_between', 'profit_target_cooldown')),
        ("VOLATILITY_BLOCK", ('atr_compressed', 'extended_run', 'compression')),
        ("DIRECTION_BLOCK", ('ema_not', 'macd_not', 'adx_too_weak', 'direction_confirmed')),
        ("PRICE_LEVEL_BLOCK", ('ema200', 'below_ema200', 'price_below')),
        ("STATE_BLOCK", ('in_trade', 'state_is')),
        ("RANGING_BLOCK", ('ranging', 'chop')),
    )

    def get_trade_summary_dataframe(self):
        if not self.trade_history:
            return pd.DataFrame()

        trades_data = []
        for trade in self.trade_history:
            tier_num = trade.get('tier', 0)
            if tier_num == 1:
                tier_name = "Tier 1"
            elif tier_num == 2:
                tier_name = "Tier 2"
    ...
    def _bucket_hold_reason(self, reason):
        reason_lower = str(reason).lower()
        for bucket_name, keywords in self._HOLD_REASON_BUCKETS:
            if any(kw in reason_lower for kw in keywords):
                return bucket_name
        return "OTHER_BLOCK"

    def _track_hold_reason(self, result):
        if not hasattr(self, '_hold_reason_counts'):
            self._hold_reason_counts = {}
            self._total_bars_checked = 0

        self._total_bars_checked += 1

        if self._pending_signal is not None:
            return

        reason = str(result[3]) if isinstance(result, tuple) and len(result) >= 4 else "unknown_result_format"
        bucketed_reason = self._bucket_hold_reason(reason)
        self._hold_reason_counts[bucketed_reason] = self._hold_reason_counts.get(bucketed_reason, 0) + 1

    def get_hold_reason_summary(self, top_n=20):
        if not getattr(self, '_hold_reason_counts', None):
            return []

        total = self._total_bars_checked or sum(self._hold_reason_counts.values())
        if total == 0:
            return []

        sorted_reasons = sorted(self._hold_reason_counts.items(), key=lambda x: x[1], reverse=True)
        return [(reason, count, round(count / total * 100, 1)) for reason, count in sorted_reasons[:top_n]]

    def _execute_entry_immediately(self, current_data, regime, is_below_ema200, idx):
        signal = self._pending_signal
        self._pending_signal = None
        self._signal_bar = -999
        self._signal_price = None

        # Use the same selective hard-block as the entry gate: only a
        # confirmed, persistent bearish regime vetoes a buy signal here.
        # (Using a blanket regime.startswith('BEARISH') check would silently
        # re-block BEARISH_WEAK even after the entry gate above lets it
        # through, defeating the point of the looser gate.)
        if self.regime_detector.is_hard_blocked(regime) and signal['decision'] == "buy":
        ...
        # Pass direction to _get_tier_config
        tier_config = self._get_tier_config(tier, self._position_direction)
        if tier_config is None:
            tier_config = self._get_tier_config(1, self._position_direction)
        stop_mult = tier_config.get('stop_mult', 2.5)

        position_mult = signal.get('position_mult', 1.0)

        decision_price = self.data.Close[-1]
        gui_pct = getattr(self, 'gui_order_size_pct', 30.0)
        size = self.calculate_position_size(
            self.equity, current_data['ATR'], decision_price,
        ...
        if signal['decision'] == "buy":
            ...
            stop = decision_price - (stop_mult * current_data['ATR'])
            if stop >= decision_price:
                return
            self.buy(size=self._bt_safe_size(size, decision_price))
        else:
            # SHORT: stop ABOVE entry
            stop = decision_price + (stop_mult * current_data['ATR'])
            if stop <= decision_price:
                return
            self.sell(size=self._bt_safe_size(size, decision_price))

        self._entry_price = decision_price
        self._stop_loss = stop
        self._entry_stop_mult = stop_mult
        self._highest_price = decision_price if self._position_direction == 'long' else None
        self._lowest_price = decision_price if self._position_direction == 'short' else None
        self._bars_held = 0
        self._partial_exits = 0
        self._entry_bar = idx
        self._entry_tier = tier
        self._entry_quality = signal['power_score']
        self._entry_confluence_score = signal.get('confluence_score', 0.0)
        self._partial_pnl_realised = 0.0
        self._trailing_activated = False
        self._trailing_stop = None
        self._be_stop_set = False
        ...
        trade_record = TradeRecord(
            trade_id=self._current_trade_id, symbol=getattr(self, 'symbol', 'SOL-USDT'),
            entry_time=bar_entry_time, entry_price=decision_price, entry_size=size,
            entry_tier=tier, entry_quality_score=signal['power_score'], entry_reason="entry_signal",
            entry_direction=self._position_direction, entry_bar=self._entry_bar,
            ...
            risk_allocation_pct=self._entry_risk_allocation_pct,
            market_regime=regime,
            original_size=size, partial_exit_details=[],
        )
        if not hasattr(self, 'trade_records'): self.trade_records = []
        self.trade_records.append(trade_record)
        self._active_trade_record = trade_record

        self._pending_entry_fill = True
        self._transition_to_in_trade()
        ...
        print(f"{direction_icon} ENTER T{tier} ({tier_names.get(tier, 'Unknown')}) "
              f"Q={signal['power_score']} @ ${decision_price:.2f} (decision close) "
              f"Size={size} Stop=${stop:.2f} {regime_icon} {regime} "
              f"Risk={self._entry_risk_allocation_pct:.2%} "
              f"[fill price will be read from engine next bar]")

    def next(self):
        self._process_pending_fills()
        self.risk_controller.update_mark_to_market(self.equity)
        ...
        if getattr(self, '_entry_time_pending', False):
            ...
            if active_record is not None:
                active_record.entry_time = true_entry_time
            self._entry_time_pending = False
        ...
        adx_pct_val = current_data.get('ADX_Percentile')
        atr_pct_val = current_data.get('ATR_Pct')
        ...
        trend_age_val = current_data.get('Trend_Age_Bearish', 0) if ema_fast_val < ema_slow_val else current_data.get(
            'Trend_Age_Bullish', 0)

        regime, confidence = self.regime_detector.detect_regime(
            ema_fast=ema_fast_val, ema_slow=ema_slow_val, adx=adx_val,
            vol_ratio=vol_ratio_val, bb_width_percentile=bb_width_pct,
            adx_percentile=adx_pct_val, atr_pct=atr_pct_val, trend_age=trend_age_val,
            is_ranging_external=current_data.get('Ranging', None))  # PATCH: shared ranging signal
        self.current_regime = regime

        if regime.startswith('BEARISH'):
            print(
                f"🐻 BEARISH REGIME: {regime} (conf={confidence:.2f}) | Price=${close_val:.2f} EMA200=${ema200_val:.2f} ADX={adx_val:.1f}")
        ...
        if ema200_val > 0 and close_val > 0:
            ...
            if is_below_ema200 and not regime.startswith('BEARISH'):
                print(f"⚠️ PRICE BELOW EMA200: ${close_val:.2f} < ${ema200_val:.2f}")
        ...
        if self.strategy_state == StrategyState.SEEKING_ENTRY:
            ...
            self._track_hold_reason(result)

            if self._pending_signal is not None:
                self._execute_entry_immediately(current_data, regime, is_below_ema200, idx)
            return
        ...
        if self.strategy_state == StrategyState.IN_TRADE:
            ...
            exit_signal, exit_pct = self.exit_manager.evaluate_exit(
                current_price=current_price, entry_price=self._entry_price, stop_loss=self._stop_loss,
                ...
                ema_fast=current_data['EMA_Fast'], ema_mid=current_data['EMA_Mid'], ema_slow=current_data['EMA_Slow'],
                macd=current_data['MACD'], macd_signal=current_data['MACD_Signal'],
                macd_prev=current_data['MACD_closed'], signal_prev=current_data['MACD_Signal_closed'],
                adx=current_data['ADX'], atr=atr, position_type=self._position_direction,
                trailing_activated=self._trailing_activated, trailing_stop=self._trailing_stop,
                tier=tier, exit_power=exit_power,
                stop_mult=getattr(self, '_entry_stop_mult', None) or tier_config.get('stop_mult')
            )
            ...
            if exit_signal:
                if exit_pct < 1.0 and self._partial_exits < 4:
                    ...
                    if current_position_size > 0:
                        ...
                        if portion >= 0.9999:
                            size_at_close = current_position_size
                            self.position.close()
                            self._pending_full_exit_fill = (exit_signal, size_at_close)
                            print(
                                f"📤 FULL EXIT {exit_signal} (partial request fully closed remaining size={size_at_close}) "
                                f"[fill price from engine next bar]")
                            return

                        self._partial_exits += 1
                        self.position.close(portion=portion)
                        self._pending_partial_exit_fill = (exit_signal, partial_size)
                        print(
                            f"📤 PARTIAL {exit_signal}: portion={portion:.2f} size={partial_size} [fill price from engine next bar]")
                        return

                size_at_close = abs(self.position.size)
                self.position.close()
                self._pending_full_exit_fill = (exit_signal, size_at_close)
                print(f"📤 FULL EXIT {exit_signal}: size={size_at_close} [fill price from engine next bar]")
                return


# ═══════════════════════════════════════════════════════════════════════════
# WALK-FORWARD OPTIMIZATION (unchanged)
# ═══════════════════════════════════════════════════════════════════════════

def reconcile_strategy_vs_engine(stats, tolerance: float = 0.01, verbose: bool = True):
    strategy = stats._strategy

    if hasattr(strategy, 'finalize_pending_fills'):
        strategy.finalize_pending_fills()

    engine_trades = stats._trades

    if not hasattr(strategy, 'trade_records'):
        if verbose: print("ℹ️  Strategy has no trade_records — skipping reconciliation")
        return True

    our_records = strategy.trade_records

    def _aggregate_engine(df):
        if len(df) == 0:
            return pd.DataFrame(columns=['EntryBar', 'PnL', 'legs'])
        g = df.groupby('EntryBar').agg(PnL=('PnL', 'sum'), legs=('PnL', 'size'))
        return g

    def _aggregate_ours(records):
        rows = [r for r in records if r.exit_time is not None and r.entry_bar is not None]
        skipped = len(records) - len(rows)
        if not rows:
            return pd.DataFrame(columns=['EntryBar', 'PnL', 'legs']), skipped
        df = pd.DataFrame({
            'EntryBar': [r.entry_bar for r in rows],
            'PnL': [r.profit for r in rows],
        })
        g = df.groupby('EntryBar').agg(PnL=('PnL', 'sum'), legs=('PnL', 'size'))
        return g, skipped

    engine_agg = _aggregate_engine(engine_trades)
    our_agg, orphaned_count = _aggregate_ours(our_records)

    if orphaned_count > 0 and verbose:
        print(f"⚠️  {orphaned_count} orphaned trade_records (no exit_time) excluded from reconciliation "
              f"— these indicate entries the engine never actually filled; investigate separately.")

    engine_entry_bars = set(engine_agg.index)
    our_entry_bars = set(our_agg.index)

    missing_in_ours = engine_entry_bars - our_entry_bars
    missing_in_engine = our_entry_bars - engine_entry_bars

    if missing_in_ours or missing_in_engine:
        print(f"\n{'=' * 70}\n❌ RECONCILIATION FAILED — ENTRY SET MISMATCH")
        print(f"   Engine real trades: {len(engine_entry_bars)}  |  Strategy real trades: {len(our_entry_bars)}")
        if missing_in_ours:
            print(f"   Entry bars in engine but missing from our ledger: {sorted(missing_in_ours)[:10]}")
        if missing_in_engine:
            print(f"   Entry bars in our ledger but never seen by engine (phantom fills): {sorted(missing_in_engine)[:10]}")
        print(f"{'=' * 70}\n")
        raise AssertionError(
            f"ENTRY SET MISMATCH: engine has {len(engine_entry_bars)} real trades, "
            f"strategy has {len(our_entry_bars)}, symmetric difference={len(missing_in_ours) + len(missing_in_engine)}.")

    engine_profit = float(engine_agg['PnL'].sum()) if len(engine_agg) else 0.0
    our_profit = float(our_agg['PnL'].sum()) if len(our_agg) else 0.0
    if abs(engine_profit - our_profit) > tolerance:
        print(f"\n{'=' * 70}\n❌ RECONCILIATION FAILED — PROFIT MISMATCH")
        print(
            f"   Engine total PnL:   ${engine_profit:.4f}\n   Strategy total PnL: ${our_profit:.4f}\n   Difference:         ${abs(engine_profit - our_profit):.4f}")
        merged = engine_agg.join(our_agg, lsuffix='_engine', rsuffix='_ours', how='outer')
        merged['diff'] = (merged['PnL_engine'] - merged['PnL_ours']).abs()
        bad = merged[merged['diff'] > 0.01].sort_values('diff', ascending=False)
        print(f"\n   Per-entry breakdown (top mismatches):")
        for entry_bar, row in bad.head(10).iterrows():
            print(f"     EntryBar={entry_bar}: engine=${row['PnL_engine']:.4f} strategy=${row['PnL_ours']:.4f} diff=${row['diff']:.4f}")
        print(f"{'=' * 70}\n")
        raise AssertionError(
            f"PROFIT MISMATCH: engine=${engine_profit:.4f}, strategy=${our_profit:.4f}, diff=${abs(engine_profit - our_profit):.4f}.")

    if len(engine_trades) > 0:
        mismatches = 0
        for i, (_, et) in enumerate(engine_trades.iterrows()):
            if i >= len(our_records):
                break
            ot = our_records[i]
            engine_entry_bar = et.get('EntryBar', None)
            if engine_entry_bar is not None and ot.entry_bar is not None:
                if int(engine_entry_bar) != int(ot.entry_bar):
                    mismatches += 1
                    if mismatches <= 3: print(
                        f"  ℹ️ (informational) leg-order difference {i}: engine EntryBar={engine_entry_bar} strategy entry_bar={ot.entry_bar}")
            engine_entry_price = et.get('EntryPrice', None)
            if engine_entry_price is not None and ot.entry_price is not None:
                if abs(float(engine_entry_price) - float(ot.entry_price)) > 0.0001:
                    mismatches += 1
                    if mismatches <= 3: print(
                        f"  ⚠️ Price mismatch trade {i}: engine EntryPrice={engine_entry_price} strategy entry_price={ot.entry_price}")
        if mismatches > 0:
            print(f"\n⚠️  TIMING/PRICE MISMATCHES: {mismatches} out of {len(engine_trades)} legs (informational)")

    if verbose:
        print(
            f"\n✅ RECONCILIATION PASSED: {len(engine_entry_bars)} real trades ({len(engine_trades)} engine legs) "
            f"| engine PnL=${engine_profit:.2f} | strategy PnL=${our_profit:.2f}")
    return True


def run_walk_forward_optimization(
        df, strategy_class=BacktestMomentumStrategy, train_bars=2160, test_bars=720,
        step_bars=None, cash=10000, commission=0.001, maximize='Sharpe Ratio',
        param_ranges=None, max_tries=200, verbose=True):
    if step_bars is None: step_bars = test_bars
    ...
    if param_ranges is None:
        ...
        optimization_map = {
            'backtest_quality_values': ['quality_tier1_min_long', 'quality_tier2_min_long'],
            'backtest_adx_values': ['tier1_adx_hard_min_long', 'tier1_adx_hard_min_short', 'tier2_adx_hard_min'],
            'backtest_rsi_values': ['tier1_rsi_min_long', 'tier2_rsi_min_long'],
            'backtest_volume_values': ['tier1_volume_min_long', 'tier1_volume_min_short', 'tier2_volume_min'],
            'backtest_momentum_values': ['tier1_momentum_min', 'tier2_momentum_min'],
            'backtest_ema_fast_values': ['ema_fast_period'],
            'backtest_ema_mid_values': ['ema_mid_period'],
    ...
    while start + train_bars + test_bars <= n:
        ...
        try:
            train_stats = bt_train.optimize(**param_ranges, maximize=maximize,
                                            max_tries=max_tries)
            reconcile_strategy_vs_engine(train_stats, tolerance=0.01, verbose=False)
        except AssertionError as e:
            if verbose:
                print(f"[Fold {fold}] ❌ TRAIN RECONCILIATION FAILED: {e}")
            raise
        except Exception as e:
            if verbose:
        ...
        try:
            reconcile_strategy_vs_engine(test_stats, tolerance=0.01, verbose=verbose)
        except AssertionError as e:
            print(f"[Fold {fold}] ❌ RECONCILIATION FAILED: {e}")
            raise
